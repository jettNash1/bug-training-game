...
export class LocaleTestingQuiz extends BaseQuiz {
    constructor() {
        console.log('[LocaleTestingQuiz] Initializing...');
        ...
        const config = {
            ...
            performanceThresholds: [
                ...
                { threshold: 80, message: '👏 Great job! You\'ve shown strong locale testing instincts!' },
        ...
        this.basicScenarios = localeTestingScenarios.basic;
        this.intermediateScenarios = localeTestingScenarios.intermediate;
        this.advancedScenarios = localeTestingScenarios.advanced;
    ...
    async startGameWhenReady() {
        console.log('[LocaleTestingQuiz] Waiting for timer settings to be loaded...');
        ...
        const checkTimerSettings = () => {
            attempts++;
            console.log(`[LocaleTestingQuiz] Check attempt ${attempts}, timePerQuestion: ${this.timePerQuestion}`);
            ...
            if (this.timePerQuestion !== undefined && this.timePerQuestion !== null) {
                console.log(`[LocaleTestingQuiz] Timer settings loaded: ${this.timePerQuestion}s per question`);
            ...
            if (attempts >= maxAttempts) {
                console.warn('[LocaleTestingQuiz] Timer settings not loaded after maximum attempts, using BaseQuiz value');
    ...
    async startGame() {
        ...
        console.log('[LocaleTestingQuiz] Starting game...');
        ...
        try {
            ...
            console.log(`[LocaleTestingQuiz] Progress loaded: ${hasProgress}`);
            ...
            if (!hasProgress) {
                ...
                console.log('[LocaleTestingQuiz] No previous progress, starting fresh');
            } else {
                ...
                if (!this.player.questionHistory || !Array.isArray(this.player.questionHistory)) {
                    console.log('[LocaleTestingQuiz] Invalid question history in loaded progress, resetting');
                    this.player.questionHistory = [];
                }
                
                // CRITICAL: Ensure currentScenario is set correctly based on question history
                this.player.currentScenario = this.player.questionHistory.length;
                console.log('[LocaleTestingQuiz] Set currentScenario to match question history:', this.player.currentScenario);
            }
            ...
            console.error('[LocaleTestingQuiz] Error starting game:', error);
            this.isLoading = false;
    ...
    initializeTimer() {
        console.log(`[LocaleTestingQuiz] Initializing timer for question ${this.player.questionHistory.length}`);
        ...
        if (!timerContainer || !timerDisplay) {
            console.error('[LocaleTestingQuiz] Timer elements not found');
            return;
        }
        
        // Check if timer is disabled (0 seconds) or timer functionality is disabled
        if (this.timerDisabled || this.timePerQuestion === 0) {
            console.log(`[LocaleTestingQuiz] Timer is disabled, hiding timer display`);
        ...
        if (restoredTime !== null) {
            ...
            console.log(`[LocaleTestingQuiz] Restored timer with ${timeLeft} seconds remaining`);
        } else {
            timeLeft = this.timePerQuestion;
            console.log(`[LocaleTestingQuiz] Starting new timer with ${timeLeft} seconds`);
        }
    ...
    handleTimedOut() {
        console.log('[LocaleTestingQuiz] Question timed out');
    ...
    displayScenario() {
        ...
        console.log(`[LocaleTestingQuiz] Displaying scenario #${this.player.currentScenario + 1}:`, {
        ...
        if (this.player.questionHistory.length > 0) {
            ...
            this.saveProgress('in-progress').catch(err => {
                console.warn('[LocaleTestingQuiz] Save after display failed:', err);
            });
        }
    }
    ...
    async handleAnswer(timedOut = false) {
        ...
        if (submitButton) {
            ...
            console.log('[LocaleTestingQuiz] Selected answer:', {
            ...
            console.error('[LocaleTestingQuiz] Error handling answer:', error);
    ...
    async endGame(failed = false) {
        console.log('[LocaleTestingQuiz] Ending game...');
        ...
        try {
            ...
            console.log('[LocaleTestingQuiz] Quiz results:', {
        ...
        if (reviewList) {
            ...
            if (recommendations) {
                ...
                if (scorePercentage >= 90) {
                    recommendationsHTML = '<p>🌟 Outstanding! You have demonstrated excellent knowledge of locale testing principles!</p>';
                    ...
                    const basicIssues = incorrectQuestions.filter(q => q.scenario.level === 'Basic');
                    const intermediateIssues = incorrectQuestions.filter(q => q.scenario.level === 'Intermediate');
                    const advancedIssues = incorrectQuestions.filter(q => q.scenario.level === 'Advanced');

                    if (basicIssues.length) {
                        recommendationsHTML += '<li>Basic Concepts:<ul>';
                        basicIssues.forEach(q => {
                            recommendationsHTML += `<li>Review ${q.scenario.title}: ${q.scenario.description}</li>`;
                        });
                        recommendationsHTML += '</ul></li>';
                    }

                    if (intermediateIssues.length) {
                        recommendationsHTML += '<li>Intermediate Concepts:<ul>';
                        intermediateIssues.forEach(q => {
                            recommendationsHTML += `<li>Review ${q.scenario.title}: ${q.scenario.description}</li>`;
                        });
                        recommendationsHTML += '</ul></li>';
                    }
                    ...
                    if (advancedIssues.length) {
                        recommendationsHTML += '<li>Advanced Concepts:<ul>';
                        advancedIssues.forEach(q => {
                            recommendationsHTML += `<li>Review ${q.scenario.title}: ${q.scenario.description}</li>`;
                        });
                        recommendationsHTML += '</ul></li>';
                    }
                    ...
                    recommendationsHTML = '<p>📚 Here are key areas for improvement in locale testing:</p><ul>';
                    // Find areas where the user made mistakes and group by level
                    const incorrectQuestions = this.player.questionHistory.filter(q => !q.isCorrect);
                    const basicIssues = incorrectQuestions.filter(q => q.scenario.level === 'Basic');
                    const intermediateIssues = incorrectQuestions.filter(q => q.scenario.level === 'Intermediate');
            ...
            console.error('[LocaleTestingQuiz] Error ending game:', error);
    ...
    // Restart the quiz
    async restartQuiz() {
        console.log('[LocaleTestingQuiz] Restarting quiz...');
    ...
    showError(message) {
        console.error('[LocaleTestingQuiz] Error:', message);
    ...
    saveTimerState(timeRemaining) {
        try {
            ...
            console.warn('[LocaleTestingQuiz] Failed to save timer state:', error);
        }
    }
    ...
    restoreTimerState() {
        try {
            ...
            if (timerData.questionIndex !== currentQuestionIndex) {
                console.log('[LocaleTestingQuiz] Timer data is for different question, ignoring');
            ...
            if (Date.now() - timerData.timestamp > maxAge) {
                console.log('[LocaleTestingQuiz] Timer data is too old, ignoring');
                localStorage.removeItem(storageKey);
                return null;
            }
            ...
            console.log(`[LocaleTestingQuiz] Restored timer state: ${timerData.timeRemaining}s remaining`);
            return timerData.timeRemaining;
            
        } catch (error) {
            console.warn('[LocaleTestingQuiz] Failed to restore timer state:', error);
            return null;
        }
    }
    ...
    clearCurrentTimerState() {
        try {
            const storageKey = this.getTimerStorageKey();
            localStorage.removeItem(storageKey);
            console.log('[LocaleTestingQuiz] Cleared current timer state');
        } catch (error) {
            console.warn('[LocaleTestingQuiz] Failed to clear timer state:', error);
        }
    }
    ...
    saveCurrentTimerState() {
        ...
        try {
            ...
            console.error('[LocaleTestingQuiz] Error saving current timer state:', error);
        }
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('[LocaleTestingQuiz] DOM loaded, initializing quiz...');
    window.localeTestingQuiz = new LocaleTestingQuiz();
}); 