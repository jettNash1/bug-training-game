...
export class ScriptMetricsTroubleshootingQuiz extends BaseQuiz {
    ...
    async startGameWhenReady() {
        ...
        const checkTimerSettings = () => {
            ...
            if (attempts >= maxAttempts) {
                console.warn('[ScriptMetricsQuiz] Timer settings not loaded after maximum attempts, using default');
    ...
    async handleAnswer(timedOut = false) {
        ...
        if (submitButton) {
            ...
            console.log('[ScriptMetricsQuiz] Selected answer:', {
            ...
            console.error('[ScriptMetricsQuiz] Error handling answer:', error);
    ...
    showError(message) {
        console.error('[ScriptMetricsQuiz] Error:', message);
    ...
    getTimerStorageKey() {
        ...
        return `script-metrics_timer_${this.player.username}_q${questionIndex}`;
    }
    ...
    saveTimerState(timeRemaining) {
        try {
            ...
            console.warn('[ScriptMetricsQuiz] Failed to save timer state:', error);
        }
    }
    ...
    restoreTimerState() {
        try {
            ...
            if (timerData.questionIndex !== currentQuestionIndex) {
                console.log('[ScriptMetricsQuiz] Timer data is for different question, ignoring');
            ...
            if (Date.now() - timerData.timestamp > maxAge) {
                console.log('[ScriptMetricsQuiz] Timer data is too old, ignoring');
                localStorage.removeItem(storageKey);
                return null;
            }
            ...
            console.log(`[ScriptMetricsQuiz] Restored timer state: ${timerData.timeRemaining}s remaining`);
            return timerData.timeRemaining;
            
        } catch (error) {
            console.warn('[ScriptMetricsQuiz] Failed to restore timer state:', error);
            return null;
        }
    }
    ...
    clearCurrentTimerState() {
        try {
            const storageKey = this.getTimerStorageKey();
            localStorage.removeItem(storageKey);
            console.log('[ScriptMetricsQuiz] Cleared current timer state');
        } catch (error) {
            console.warn('[ScriptMetricsQuiz] Failed to clear timer state:', error);
        }
    }
    ...
    clearAllTimerStates() {
        try {
            ...
            const prefix = `script-metrics_timer_${this.player.username}_`;
            ...
            console.log('[ScriptMetricsQuiz] Cleared all timer states');
        } catch (error) {
            console.warn('[ScriptMetricsQuiz] Failed to clear timer states:', error);
        }
    }
