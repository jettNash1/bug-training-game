...
export class ScriptMetricsTroubleshootingQuiz extends BaseQuiz {
    ...
    async handleAnswer(timedOut = false) {
        ...
        if (submitButton) {
            ...
            this.saveProgress().catch(err => {
                console.error('[ScriptMetricsTroubleshootingQuiz] Failed to save progress:', err);
            });
    ...
    getTimerStorageKey() {
        ...
        return `script-metrics_timer_${username}_q${questionIndex}`;
    }
    ...
    clearAllTimerStates() {
        try {
            const username = localStorage.getItem('username') || 'anonymous';
            const prefix = `script-metrics_timer_${username}_`;
            ...
            console.log(`[ScriptMetricsQuiz] Cleared ${keysToRemove.length} timer states`);
        } catch (error) {
    ...
    saveCurrentTimerState() {
        ...
        try {
            ...
            console.error('[ScriptMetricsQuiz] Error saving current timer state:', error);
        }
    }
}
