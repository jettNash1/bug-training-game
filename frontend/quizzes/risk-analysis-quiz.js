...
import { riskAnalysisScenarios } from '../data/riskAnalysis-scenarios.js';

export class RiskAnalysisQuiz extends BaseQuiz {
    constructor() {
        console.log('[RiskAnalysisQuiz] Initializing...');
        ...
        const config = {
            ...
            performanceThresholds: [
                ...
                { threshold: 80, message: '👏 Great job! You\'ve shown strong risk analysis instincts!' },
        ...
        this.basicScenarios = riskAnalysisScenarios.basic;
        this.intermediateScenarios = riskAnalysisScenarios.intermediate;
        this.advancedScenarios = riskAnalysisScenarios.advanced;
    ...
    async startGameWhenReady() {
        console.log('[RiskAnalysisQuiz] Waiting for timer settings to be loaded...');
        ...
        const checkTimerSettings = () => {
            attempts++;
            console.log(`[RiskAnalysisQuiz] Check attempt ${attempts}, timePerQuestion: ${this.timePerQuestion}`);
            ...
            if (this.timePerQuestion !== undefined && this.timePerQuestion !== null) {
                console.log(`[RiskAnalysisQuiz] Timer settings loaded: ${this.timePerQuestion}s per question`);
            ...
            if (attempts >= maxAttempts) {
                console.warn('[RiskAnalysisQuiz] Timer settings not loaded after maximum attempts, using BaseQuiz value');
    ...
    async startGame() {
        ...
        try {
            ...
            console.log(`[RiskAnalysisQuiz] Progress loaded: ${hasProgress}`);
            ...
            if (!hasProgress) {
                ...
                if (!this.player.questionHistory || !Array.isArray(this.player.questionHistory)) {
                    console.log('[RiskAnalysisQuiz] Invalid question history in loaded progress, resetting');
                    this.player.questionHistory = [];
                }
                
                // CRITICAL: Ensure currentScenario is set correctly based on question history
                this.player.currentScenario = this.player.questionHistory.length;
                console.log('[RiskAnalysisQuiz] Set currentScenario to match question history:', this.player.currentScenario);
            }
    ...
    initializeTimer() {
        console.log(`[RiskAnalysisQuiz] Initializing timer for question ${this.player.questionHistory.length}`);
        ...
        if (!timerContainer || !timerDisplay) {
            console.error('[RiskAnalysisQuiz] Timer elements not found');
            return;
        }
        
        // Check if timer is disabled (0 seconds) or timer functionality is disabled
        if (this.timerDisabled || this.timePerQuestion === 0) {
            console.log(`[RiskAnalysisQuiz] Timer is disabled, hiding timer display`);
        ...
        if (restoredTime !== null) {
            ...
            console.log(`[RiskAnalysisQuiz] Restored timer with ${timeLeft} seconds remaining`);
        } else {
            timeLeft = this.timePerQuestion;
            console.log(`[RiskAnalysisQuiz] Starting new timer with ${timeLeft} seconds`);
        }
    ...
    handleTimedOut() {
        console.log('[RiskAnalysisQuiz] Question timed out');
    ...
    displayScenario() {
        ...
        console.log(`[RiskAnalysisQuiz] Displaying scenario #${this.player.currentScenario + 1}:`, {
        ...
        if (this.player.questionHistory.length > 0) {
            ...
            this.saveProgress('in-progress').catch(err => {
                console.warn('[RiskAnalysisQuiz] Save after display failed:', err);
            });
        }
    }
    ...
    async handleAnswer(timedOut = false) {
        ...
        if (submitButton) {
            ...
            console.log('[RiskAnalysisQuiz] Selected answer:', {
            ...
            console.error('[RiskAnalysisQuiz] Error handling answer:', error);
    ...
    async endGame(failed = false) {
        console.log('[RiskAnalysisQuiz] Ending game...');
        ...
        try {
            ...
            console.log('[RiskAnalysisQuiz] Quiz results:', {
        ...
        if (reviewList) {
            ...
            if (recommendations) {
                ...
                if (scorePercentage >= 90) {
                    recommendationsHTML = '<p>🌟 Outstanding! You have demonstrated excellent knowledge of risk analysis principles!</p>';
            ...
            console.error('[RiskAnalysisQuiz] Error ending game:', error);
    ...
    // Restart the quiz
    async restartQuiz() {
        console.log('[RiskAnalysisQuiz] Restarting quiz...');
    ...
    showError(message) {
        console.error('[RiskAnalysisQuiz] Error:', message);
    ...
    saveTimerState(timeRemaining) {
        try {
            ...
            console.warn('[RiskAnalysisQuiz] Failed to save timer state:', error);
        }
    }
    ...
    restoreTimerState() {
        try {
            ...
            if (timerData.questionIndex !== currentQuestionIndex) {
                console.log('[RiskAnalysisQuiz] Timer data is for different question, ignoring');
            ...
            if (Date.now() - timerData.timestamp > maxAge) {
                console.log('[RiskAnalysisQuiz] Timer data is too old, ignoring');
                localStorage.removeItem(storageKey);
                return null;
            }
            ...
            console.log(`[RiskAnalysisQuiz] Restored timer state: ${timerData.timeRemaining}s remaining`);
            return timerData.timeRemaining;
            
        } catch (error) {
            console.warn('[RiskAnalysisQuiz] Failed to restore timer state:', error);
            return null;
        }
    }
    ...
    clearCurrentTimerState() {
        try {
            const storageKey = this.getTimerStorageKey();
            localStorage.removeItem(storageKey);
            console.log('[RiskAnalysisQuiz] Cleared current timer state');
        } catch (error) {
            console.warn('[RiskAnalysisQuiz] Failed to clear timer state:', error);
        }
    }
    ...
    saveCurrentTimerState() {
        ...
        try {
            ...
            console.error('[RiskAnalysisQuiz] Error saving current timer state:', error);
        }
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('[RiskAnalysisQuiz] DOM loaded, initializing quiz...');
    window.riskAnalysisQuiz = new RiskAnalysisQuiz();
}); 