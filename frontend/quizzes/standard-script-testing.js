...
import { standardScriptScenarios } from '../data/standardScript-scenarios.js';
...
export class StandardScriptTestingQuiz extends BaseQuiz {
    constructor() {
        ...
        this.basicScenarios = standardScriptScenarios.basic;
        this.intermediateScenarios = standardScriptScenarios.intermediate;
        this.advancedScenarios = standardScriptScenarios.advanced;
    ...
    async startGameWhenReady() {
        console.log('[StandardScriptTestingQuiz] Waiting for timer settings to be loaded...');
        ...
        const checkTimerSettings = () => {
            attempts++;
            console.log(`[StandardScriptTestingQuiz] Check attempt ${attempts}, timePerQuestion: ${this.timePerQuestion}`);
            ...
            if (this.timePerQuestion !== undefined && this.timePerQuestion !== null) {
                console.log(`[StandardScriptTestingQuiz] Timer settings loaded: ${this.timePerQuestion}s per question`);
            ...
            if (attempts >= maxAttempts) {
                console.warn('[StandardScriptTestingQuiz] Timer settings not loaded after maximum attempts, using BaseQuiz value');
    ...
    initializeTimer() {
        console.log(`[StandardScriptTestingQuiz] Initializing timer for question ${this.player.questionHistory.length}`);
        ...
        if (this.timerDisabled || this.timePerQuestion === 0) {
            console.log(`[StandardScriptTestingQuiz] Timer is disabled, hiding timer display`);
        ...
        if (restoredTime !== null) {
            ...
            console.log(`[StandardScriptTestingQuiz] Restored timer with ${timeLeft} seconds remaining`);
        } else {
            timeLeft = this.timePerQuestion;
            console.log(`[StandardScriptTestingQuiz] Starting new timer with ${timeLeft} seconds`);
        }
    ...
    async handleAnswer(timedOut = false) {
        ...
        if (submitButton) {
            ...
            this.saveProgress().catch(err => {
                console.error('[StandardScriptTestingQuiz] Failed to save progress:', err);
            });
    ...
    getTimerStorageKey() {
        ...
        return `standard-script_timer_${username}_q${questionIndex}`;
    }
    ...
    saveTimerState(timeRemaining) {
        try {
            ...
            console.warn('[StandardScriptTestingQuiz] Failed to save timer state:', error);
        }
    }
    ...
    restoreTimerState() {
        try {
            ...
            if (timerData.questionIndex !== currentQuestionIndex) {
                console.log('[StandardScriptTestingQuiz] Timer data is for different question, ignoring');
            ...
            if (Date.now() - timerData.timestamp > maxAge) {
                console.log('[StandardScriptTestingQuiz] Timer data is too old, ignoring');
                localStorage.removeItem(storageKey);
                return null;
            }
            ...
            console.log(`[StandardScriptTestingQuiz] Restored timer state: ${timerData.timeRemaining}s remaining`);
            return timerData.timeRemaining;
            
        } catch (error) {
            console.warn('[StandardScriptTestingQuiz] Failed to restore timer state:', error);
            return null;
        }
    }
    ...
    clearCurrentTimerState() {
        try {
            const storageKey = this.getTimerStorageKey();
            localStorage.removeItem(storageKey);
            console.log('[StandardScriptTestingQuiz] Cleared current timer state');
        } catch (error) {
            console.warn('[StandardScriptTestingQuiz] Failed to clear timer state:', error);
        }
    }
    ...
    clearAllTimerStates() {
        try {
            const username = localStorage.getItem('username') || 'anonymous';
            const prefix = `standard-script_timer_${username}_`;
            ...
            console.log(`[StandardScriptTestingQuiz] Cleared ${keysToRemove.length} timer states`);
        } catch (error) {
            console.warn('[StandardScriptTestingQuiz] Failed to clear timer states:', error);
        }
    }
    ...
    saveCurrentTimerState() {
        ...
        try {
            ...
            console.error('[StandardScriptTestingQuiz] Error saving current timer state:', error);
        }
    }
}
