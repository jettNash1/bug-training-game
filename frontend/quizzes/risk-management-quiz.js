...
export class RiskManagementQuiz extends BaseQuiz {
    constructor() {
        console.log('[RiskManagementQuiz] Initializing...');
        ...
        const config = {
            ...
            performanceThresholds: [
                ...
                { threshold: 80, message: '👏 Great job! You\'ve shown strong risk management instincts!' },
        ...
        this.basicScenarios = riskManagementScenarios.basic;
        this.intermediateScenarios = riskManagementScenarios.intermediate;
        this.advancedScenarios = riskManagementScenarios.advanced;
    ...
    async startGameWhenReady() {
        console.log('[RiskManagementQuiz] Waiting for timer settings to be loaded...');
        ...
        const checkTimerSettings = () => {
            attempts++;
            console.log(`[RiskManagementQuiz] Check attempt ${attempts}, timePerQuestion: ${this.timePerQuestion}`);
            ...
            if (this.timePerQuestion !== undefined && this.timePerQuestion !== null) {
                console.log(`[RiskManagementQuiz] Timer settings loaded: ${this.timePerQuestion}s per question`);
            ...
            if (attempts >= maxAttempts) {
                console.warn('[RiskManagementQuiz] Timer settings not loaded after maximum attempts, using BaseQuiz value');
    ...
    async startGame() {
        ...
        console.log('[RiskManagementQuiz] Starting game...');
        ...
        try {
            ...
            console.log(`[RiskManagementQuiz] Progress loaded: ${hasProgress}`);
            ...
            if (!hasProgress) {
                ...
                console.log('[RiskManagementQuiz] No previous progress, starting fresh');
            } else {
                ...
                if (!this.player.questionHistory || !Array.isArray(this.player.questionHistory)) {
                    console.log('[RiskManagementQuiz] Invalid question history in loaded progress, resetting');
                    this.player.questionHistory = [];
                }
                
                // CRITICAL: Ensure currentScenario is set correctly based on question history
                this.player.currentScenario = this.player.questionHistory.length;
                console.log('[RiskManagementQuiz] Set currentScenario to match question history:', this.player.currentScenario);
            }
            ...
            console.error('[RiskManagementQuiz] Error starting game:', error);
            this.isLoading = false;
    ...
    initializeTimer() {
        console.log(`[RiskManagementQuiz] Initializing timer for question ${this.player.questionHistory.length}`);
        ...
        if (!timerContainer || !timerDisplay) {
            console.error('[RiskManagementQuiz] Timer elements not found');
            return;
        }
        
        // Check if timer is disabled (0 seconds) or timer functionality is disabled
        if (this.timerDisabled || this.timePerQuestion === 0) {
            console.log(`[RiskManagementQuiz] Timer is disabled, hiding timer display`);
        ...
        if (restoredTime !== null) {
            ...
            console.log(`[RiskManagementQuiz] Restored timer with ${timeLeft} seconds remaining`);
        } else {
            timeLeft = this.timePerQuestion;
            console.log(`[RiskManagementQuiz] Starting new timer with ${timeLeft} seconds`);
        }
    ...
    handleTimedOut() {
        console.log('[RiskManagementQuiz] Question timed out');
    ...
    displayScenario() {
        ...
        console.log(`[RiskManagementQuiz] Displaying scenario #${this.player.currentScenario + 1}:`, {
        ...
        if (this.player.questionHistory.length > 0) {
            ...
            this.saveProgress('in-progress').catch(err => {
                console.warn('[RiskManagementQuiz] Save after display failed:', err);
            });
        }
    }
    ...
    async handleAnswer(timedOut = false) {
        ...
        if (submitButton) {
            ...
            console.error('[RiskManagementQuiz] Error handling answer:', error);
    ...
    async endGame(failed = false) {
        console.log('[RiskManagementQuiz] Ending game...');
        ...
        try {
            ...
            console.log('[RiskManagementQuiz] Quiz results:', {
        ...
        if (reviewList) {
            ...
            if (recommendations) {
                ...
                if (scorePercentage >= 90) {
                    recommendationsHTML = '<p>🌟 Outstanding! You have demonstrated excellent knowledge of risk management principles!</p>';
                    ...
                    incorrectQuestions.forEach(q => {
                        ...
                        const toolRec = q.selectedAnswer.tool ? 
                            `Consider using the ${q.selectedAnswer.tool} tool.` : '';
                        recommendationsHTML += `<li>Review ${q.scenario.title} (${q.scenario.level} level): ${q.scenario.description} ${toolRec}</li>`;
                    });
            ...
            console.error('[RiskManagementQuiz] Error ending game:', error);
    ...
    // Restart the quiz
    async restartQuiz() {
        console.log('[RiskManagementQuiz] Restarting quiz...');
    ...
    showError(message) {
        console.error('[RiskManagementQuiz] Error:', message);
    ...
    getTimerStorageKey() {
        ...
        return `risk-management_timer_${username}_q${questionIndex}`;
    }
    ...
    saveTimerState(timeRemaining) {
        try {
            ...
            console.warn('[RiskManagementQuiz] Failed to save timer state:', error);
        }
    }
    ...
    restoreTimerState() {
        try {
            ...
            if (timerData.questionIndex !== currentQuestionIndex) {
                console.log('[RiskManagementQuiz] Timer data is for different question, ignoring');
            ...
            if (Date.now() - timerData.timestamp > maxAge) {
                console.log('[RiskManagementQuiz] Timer data is too old, ignoring');
                localStorage.removeItem(storageKey);
                return null;
            }
            ...
            console.log(`[RiskManagementQuiz] Restored timer state: ${timerData.timeRemaining}s remaining`);
            return timerData.timeRemaining;
            
        } catch (error) {
            console.warn('[RiskManagementQuiz] Failed to restore timer state:', error);
            return null;
        }
    }
    ...
    clearCurrentTimerState() {
        try {
            const storageKey = this.getTimerStorageKey();
            localStorage.removeItem(storageKey);
            console.log('[RiskManagementQuiz] Cleared current timer state');
        } catch (error) {
            console.warn('[RiskManagementQuiz] Failed to clear timer state:', error);
        }
    }
    ...
    clearAllTimerStates() {
        try {
            const username = localStorage.getItem('username') || 'anonymous';
            const prefix = `risk-management_timer_${username}_`;
            ...
            console.log(`[RiskManagementQuiz] Cleared ${keysToRemove.length} timer states`);
        } catch (error) {
            console.warn('[RiskManagementQuiz] Failed to clear timer states:', error);
        }
    }
    ...
    saveCurrentTimerState() {
        ...
        try {
            ...
            console.error('[RiskManagementQuiz] Error saving current timer state:', error);
        }
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('[RiskManagementQuiz] DOM loaded, initializing quiz...');
    window.riskManagementQuiz = new RiskManagementQuiz();
}); 