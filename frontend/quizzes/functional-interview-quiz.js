...
export class FunctionalInterviewQuiz extends BaseQuiz {
    constructor() {
        const config = {
            ...
            performanceThresholds: [
                ...
                { threshold: 80, message: '👏 Great job! You\'ve shown strong functional interview skills!' },
    ...
    async endGame(isFailed = false) {
        ...
        title.textContent = passed ? 'Quiz Completed!' : 'Quiz Failed';
        resultsContainer.appendChild(title);
        
        // Add score
        const score = document.createElement('p');
        score.textContent = `Your Score: ${scorePercentage}%`;
        resultsContainer.appendChild(score);
        
        // Add feedback message
        const feedback = document.createElement('p');
        feedback.textContent = this.getFeedbackMessage(scorePercentage);
        resultsContainer.appendChild(feedback);
        
        // Add question review
        const reviewContainer = document.createElement('div');
        reviewContainer.className = 'question-review';
        reviewContainer.innerHTML = '<h3>Question Review</h3>';
        
        this.player.questionHistory.forEach((question, index) => {
            const questionDiv = document.createElement('div');
            questionDiv.className = `question-review-item ${question.isCorrect ? 'correct' : 'incorrect'}`;
            questionDiv.innerHTML = `
                <p><strong>Question ${index + 1}:</strong> ${question.question}</p>
                <p><strong>Your Answer:</strong> ${question.selectedAnswer}</p>
                <p><strong>Result:</strong> ${question.isCorrect ? 'Correct' : 'Incorrect'}</p>
            `;
    ...
    getFeedbackMessage(scorePercentage) {
        if (scorePercentage >= 90) {
            return 'Excellent work! You have demonstrated a strong understanding of functional interview concepts.';
        } else if (scorePercentage >= 80) {
            return 'Great job! You have a good grasp of functional interview concepts.';
        } else if (scorePercentage >= 70) {
            return 'Good effort! You have a basic understanding of functional interview concepts.';
        } else {
    ...
    clearQuizLocalStorage() {
        localStorage.removeItem(`quiz_progress_${this.username}_${this.quizName}`);
    }
