...
import { functionalInterviewScenarios } from '../data/functionalInterview-scenarios.js';

export class FunctionalInterviewQuiz extends BaseQuiz {
    constructor() {
        console.log('[Functional Interview Quiz] Initializing...');
        ...
        const config = {
            ...
            performanceThresholds: [
                { threshold: 90, message: '🏆 Outstanding! You\'re a functional interview expert!' },
                { threshold: 80, message: '👏 Great job! You\'ve shown strong functional interview instincts!' },
            ...
            quizTitle: 'Functional Testing',
            quizDescription: 'Review your understanding of functional testing with comprehensive scenarios and questions. Master functional testing concepts, and methodologies.'
        };
        ...
        this.basicScenarios = functionalInterviewScenarios.basic;
        this.intermediateScenarios = functionalInterviewScenarios.intermediate;
        this.advancedScenarios = functionalInterviewScenarios.advanced;
    ...
    async startGameWhenReady() {
        console.log('[FunctionalInterviewQuiz] Waiting for timer settings to be loaded...');
        ...
        const checkTimerSettings = () => {
            attempts++;
            console.log(`[FunctionalInterviewQuiz] Check attempt ${attempts}, timePerQuestion: ${this.timePerQuestion}`);
            ...
            if (this.timePerQuestion !== undefined && this.timePerQuestion !== null) {
                console.log(`[FunctionalInterviewQuiz] Timer settings loaded: ${this.timePerQuestion}s per question`);
            ...
            if (attempts >= maxAttempts) {
                console.warn('[FunctionalInterviewQuiz] Timer settings not loaded after maximum attempts, using BaseQuiz value');
    ...
    async startGame() {
        ...
        try {
            ...
            console.log(`[Functional Interview Quiz] Progress loaded: ${hasProgress}`);
            ...
            if (!hasProgress) {
                ...
                console.log('[Functional Interview Quiz] No previous progress, starting fresh');
            } else {
                ...
                if (!this.player.questionHistory || !Array.isArray(this.player.questionHistory)) {
                    console.log('[Functional Interview Quiz] Invalid question history in loaded progress, resetting');
                    this.player.questionHistory = [];
                }
                
                // CRITICAL: Ensure currentScenario is set correctly based on question history
                this.player.currentScenario = this.player.questionHistory.length;
                console.log('[Functional Interview Quiz] Set currentScenario to match question history:', this.player.currentScenario);
            }
            ...
            console.error('[Functional Interview Quiz] Error starting game:', error);
            this.isLoading = false;
    ...
    initializeTimer() {
        console.log(`[FunctionalInterviewQuiz] Initializing timer for question ${this.player.questionHistory.length}`);
        ...
        if (this.timerDisabled || this.timePerQuestion === 0) {
            console.log(`[FunctionalInterviewQuiz] Timer is disabled, hiding timer display`);
        ...
        if (restoredTime !== null) {
            ...
            console.log(`[FunctionalInterviewQuiz] Restored timer with ${timeLeft} seconds remaining`);
        } else {
            timeLeft = this.timePerQuestion;
            console.log(`[FunctionalInterviewQuiz] Starting new timer with ${timeLeft} seconds`);
        }
    ...
    handleTimedOut() {
        console.log('[Functional Interview Quiz] Question timed out');
    ...
    displayScenario() {
        ...
        console.log(`[Functional Interview Quiz] Displaying scenario #${this.player.currentScenario + 1}:`, {
        ...
        if (this.player.questionHistory.length > 0) {
            ...
            this.saveProgress('in-progress').catch(err => {
                console.warn('[Functional Interview Quiz] Save after display failed:', err);
            });
        }
    }
    ...
    async handleAnswer(timedOut = false) {
        ...
        if (submitButton) {
            ...
            console.log('[Functional Interview Quiz] Selected answer:', {
            ...
            console.error('[Functional Interview Quiz] Error handling answer:', error);
    ...
    async endGame(failed = false) {
        console.log('[Functional Interview Quiz] Ending game...');
        ...
        try {
            ...
            console.log('[Functional Interview Quiz] Quiz results:', {
        ...
        if (reviewList) {
            ...
            if (recommendations) {
                ...
                if (scorePercentage >= 90) {
                    recommendationsHTML = '<p>🌟 Outstanding! You have demonstrated excellent knowledge of testing principles across basic, intermediate and advanced concepts!</p>';
                    ...
                    Object.entries(groupedByLevel).forEach(([level, questions]) => {
                        recommendationsHTML += `<li><strong>${level} Level Concepts:</strong><ul>`;
                        questions.forEach(q => {
                            recommendationsHTML += `<li>${q.scenario.title}: ${q.scenario.description}`;
                            if (q.selectedAnswer && q.selectedAnswer.outcome) {
                                recommendationsHTML += `<br><em>Feedback: ${q.selectedAnswer.outcome}</em>`;
                            }
            ...
            console.error('[Functional Interview Quiz] Error ending game:', error);
    ...
    // Restart the quiz
    async restartQuiz() {
        console.log('[Functional Interview Quiz] Restarting quiz...');
    ...
    showError(message) {
        console.error('[FunctionalInterviewQuiz] Error:', message);
    ...
    getTimerStorageKey() {
        ...
        return `functional-interview_timer_${username}_q${questionIndex}`;
    }
    ...
    saveTimerState(timeRemaining) {
        try {
            ...
            console.warn('[FunctionalInterviewQuiz] Failed to save timer state:', error);
        }
    }
    ...
    restoreTimerState() {
        try {
            ...
            if (timerData.questionIndex !== currentQuestionIndex) {
                console.log('[FunctionalInterviewQuiz] Timer data is for different question, ignoring');
            ...
            if (Date.now() - timerData.timestamp > maxAge) {
                console.log('[FunctionalInterviewQuiz] Timer data is too old, ignoring');
                localStorage.removeItem(storageKey);
                return null;
            }
            ...
            console.log(`[FunctionalInterviewQuiz] Restored timer state: ${timerData.timeRemaining}s remaining`);
            return timerData.timeRemaining;
            
        } catch (error) {
            console.warn('[FunctionalInterviewQuiz] Failed to restore timer state:', error);
            return null;
        }
    }
    ...
    clearCurrentTimerState() {
        try {
            const storageKey = this.getTimerStorageKey();
            localStorage.removeItem(storageKey);
            console.log('[FunctionalInterviewQuiz] Cleared current timer state');
        } catch (error) {
            console.warn('[FunctionalInterviewQuiz] Failed to clear timer state:', error);
        }
    }
    ...
    clearAllTimerStates() {
        try {
            const username = localStorage.getItem('username') || 'anonymous';
            const prefix = `functional-interview_timer_${username}_`;
            ...
            console.log(`[FunctionalInterviewQuiz] Cleared ${keysToRemove.length} timer states`);
        } catch (error) {
            console.warn('[FunctionalInterviewQuiz] Failed to clear timer states:', error);
        }
    }
    ...
    saveCurrentTimerState() {
        ...
        try {
            ...
            console.error('[FunctionalInterviewQuiz] Error saving current timer state:', error);
        }
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('[Functional Interview Quiz] DOM loaded, initializing quiz...');
    window.functionalInterviewQuiz = new FunctionalInterviewQuiz();
}); 