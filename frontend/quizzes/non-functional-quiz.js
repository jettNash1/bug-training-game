...
import { nonFunctionalScenarios } from '../data/nonFunctional-scenarios.js';

export class NonFunctionalTestingQuiz extends BaseQuiz {
    constructor() {
        console.log('[Non-Functional Testing Quiz] Initializing...');
        ...
        const config = {
            ...
            performanceThresholds: [
                ...
                { threshold: 80, message: '👏 Great job! You\'ve shown strong non-functional testing instincts!' },
        ...
        this.basicScenarios = nonFunctionalScenarios.basic;
        this.intermediateScenarios = nonFunctionalScenarios.intermediate;
        this.advancedScenarios = nonFunctionalScenarios.advanced;
    ...
    async startGameWhenReady() {
        ...
        const checkTimerSettings = () => {
            ...
            if (attempts >= maxAttempts) {
                console.warn('[NonFunctionalQuiz] Timer settings not loaded after maximum attempts, using BaseQuiz value');
    ...
    initializeTimer() {
        console.log(`[NonFunctionalQuiz] Initializing timer for question ${this.player.questionHistory.length}`);
        ...
        if (!timerContainer || !timerDisplay) {
            console.error('[NonFunctionalQuiz] Timer elements not found');
            return;
        }
        
        // Check if timer is disabled (0 seconds) or timer functionality is disabled
        if (this.timerDisabled || this.timePerQuestion === 0) {
            console.log(`[NonFunctionalQuiz] Timer is disabled, hiding timer display`);
        ...
        if (restoredTime !== null) {
            ...
            console.log(`[NonFunctionalQuiz] Restored timer with ${timeLeft} seconds remaining`);
        } else {
            timeLeft = this.timePerQuestion;
            console.log(`[NonFunctionalQuiz] Starting new timer with ${timeLeft} seconds`);
        }
    ...
    handleTimedOut() {
        console.log('[Non-Functional Testing Quiz] Question timed out');
    ...
    displayScenario() {
        ...
        console.log(`[Non-Functional Testing Quiz] Displaying scenario #${this.player.currentScenario + 1}:`, {
    ...
    async endGame(failed = false) {
        console.log('[Non-Functional Testing Quiz] Ending game...');
        ...
        try {
            ...
            console.log('[Non-Functional Testing Quiz] Quiz results:', {
        ...
        if (reviewList) {
            ...
            if (recommendations) {
                ...
                if (scorePercentage >= 90) {
                    recommendationsHTML = '<p>🌟 Outstanding! You have demonstrated excellent knowledge of non-functional testing principles and verification practices!</p>';
            ...
            console.error('[Non-Functional Testing Quiz] Error ending game:', error);
    ...
    // Restart the quiz
    async restartQuiz() {
        console.log('[Non-Functional Testing Quiz] Restarting quiz...');
    ...
    getTimerStorageKey() {
        ...
        return `non-functional_timer_${username}_q${questionIndex}`;
    }
    ...
    clearAllTimerStates() {
        try {
            const username = localStorage.getItem('username') || 'anonymous';
            const prefix = `non-functional_timer_${username}_`;
            ...
            console.log(`[NonFunctionalQuiz] Cleared ${keysToRemove.length} timer states`);
        } catch (error) {
    ...
    saveCurrentTimerState() {
        ...
        try {
            ...
            console.error('[NonFunctionalQuiz] Error saving current timer state:', error);
        }
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('[Non-Functional Testing Quiz] DOM loaded, initializing quiz...');
    window.nonFunctionalTestingQuiz = new NonFunctionalTestingQuiz();
}); 