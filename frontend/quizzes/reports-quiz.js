...
import { reportsScenarios } from '../data/reports-scenarios.js';

export class ReportsQuiz extends BaseQuiz {
    constructor() {
        console.log('[ReportsQuiz] Initializing...');
        ...
        const config = {
            ...
            performanceThresholds: [
                ...
                { threshold: 80, message: '👏 Great job! You\'ve shown strong reports instincts!' },
        ...
        this.basicScenarios = reportsScenarios.basic;
        this.intermediateScenarios = reportsScenarios.intermediate;
        this.advancedScenarios = reportsScenarios.advanced;
    ...
    async startGameWhenReady() {
        console.log('[ReportsQuiz] Waiting for timer settings to be loaded...');
        ...
        const checkTimerSettings = () => {
            attempts++;
            console.log(`[ReportsQuiz] Check attempt ${attempts}, timePerQuestion: ${this.timePerQuestion}`);
            ...
            if (this.timePerQuestion !== undefined && this.timePerQuestion !== null) {
                console.log(`[ReportsQuiz] Timer settings loaded: ${this.timePerQuestion}s per question`);
            ...
            if (attempts >= maxAttempts) {
                console.warn('[ReportsQuiz] Timer settings not loaded after maximum attempts, using BaseQuiz value');
    ...
    async startGame() {
        ...
        try {
            ...
            console.log(`[ReportsQuiz] Progress loaded: ${hasProgress}`);
            ...
            if (!hasProgress) {
                ...
                console.log('[ReportsQuiz] No previous progress, starting fresh');
            } else {
                ...
                if (!this.player.questionHistory || !Array.isArray(this.player.questionHistory)) {
                    console.log('[ReportsQuiz] Invalid question history in loaded progress, resetting');
                    this.player.questionHistory = [];
                }
                
                // CRITICAL: Ensure currentScenario is set correctly based on question history
                this.player.currentScenario = this.player.questionHistory.length;
                console.log('[ReportsQuiz] Set currentScenario to match question history:', this.player.currentScenario);
            }
    ...
    initializeTimer() {
        console.log(`[ReportsQuiz] Initializing timer for question ${this.player.questionHistory.length}`);
        ...
        if (restoredTime !== null) {
            ...
            console.log(`[ReportsQuiz] Restored timer with ${timeLeft} seconds remaining`);
        } else {
            timeLeft = this.timePerQuestion;
            console.log(`[ReportsQuiz] Starting new timer with ${timeLeft} seconds`);
        }
    ...
    handleTimedOut() {
        console.log('[ReportsQuiz] Question timed out');
    ...
    displayScenario() {
        ...
        console.log(`[ReportsQuiz] Displaying scenario #${this.player.currentScenario + 1}:`, {
        ...
        if (this.player.questionHistory.length > 0) {
            ...
            this.saveProgress('in-progress').catch(err => {
                console.warn('[ReportsQuiz] Save after display failed:', err);
            });
        }
    }
    ...
    async handleAnswer(timedOut = false) {
        ...
        if (submitButton) {
            ...
            console.log('[ReportsQuiz] Selected answer:', {
            ...
            console.error('[ReportsQuiz] Error handling answer:', error);
    ...
    async endGame(failed = false) {
        console.log('[ReportsQuiz] Ending game...');
        ...
        try {
            ...
            console.log('[ReportsQuiz] Quiz results:', {
        ...
        if (reviewList) {
            ...
            if (recommendations) {
                ...
                if (scorePercentage >= 90) {
                    recommendationsHTML = '<p>🌟 Outstanding! You have demonstrated excellent knowledge of issue verification principles!</p>';
                    ...
                    Object.entries(groupedByLevel).forEach(([level, questions]) => {
                        ...
                        questions.forEach(q => {
                            ...
                            if (q.scenario.tool) {
                                recommendationsHTML += `<li class="tool-tip">💡 Focus on improving your ${q.scenario.tool} skills</li>`;
                            }
                        });
                    ...
                    // Display recommendations by level with emphasis on basic concepts
                    Object.entries(groupedByLevel).forEach(([level, questions]) => {
                        recommendationsHTML += `<li><strong>${level} Level:</strong><ul>`;
                        questions.forEach(q => {
                            recommendationsHTML += `<li>Review ${q.scenario.title}: ${q.scenario.description}</li>`;
                            if (q.scenario.tool) {
                                recommendationsHTML += `<li class="tool-tip">💡 Focus on mastering ${q.scenario.tool} fundamentals</li>`;
                            }
                        });
                        recommendationsHTML += '</ul></li>';
                    });
                    recommendationsHTML += '</ul><p>Focus on mastering the Basic level concepts before moving to more advanced topics.</p>';
                }
            ...
            console.error('[ReportsQuiz] Error ending game:', error);
    ...
    // Restart the quiz
    async restartQuiz() {
        console.log('[ReportsQuiz] Restarting quiz...');
    ...
    showError(message) {
        console.error('[ReportsQuiz] Error:', message);
    ...
    getTimerStorageKey() {
        ...
        return `reports_timer_${this.player.username}_q${questionIndex}`;
    }
    ...
    saveTimerState(timeRemaining) {
        try {
            ...
            console.warn('[ReportsQuiz] Failed to save timer state:', error);
        }
    }
    ...
    restoreTimerState() {
        try {
            ...
            if (timerData.questionIndex !== currentQuestionIndex) {
                console.log('[ReportsQuiz] Timer data is for different question, ignoring');
            ...
            if (Date.now() - timerData.timestamp > maxAge) {
                console.log('[ReportsQuiz] Timer data is too old, ignoring');
                localStorage.removeItem(storageKey);
                return null;
            }
            ...
            console.log(`[ReportsQuiz] Restored timer state: ${timerData.timeRemaining}s remaining`);
            return timerData.timeRemaining;
            
        } catch (error) {
            console.warn('[ReportsQuiz] Failed to restore timer state:', error);
            return null;
        }
    }
    ...
    clearCurrentTimerState() {
        try {
            const storageKey = this.getTimerStorageKey();
            localStorage.removeItem(storageKey);
            console.log('[ReportsQuiz] Cleared current timer state');
        } catch (error) {
            console.warn('[ReportsQuiz] Failed to clear timer state:', error);
        }
    }
    ...
    clearAllTimerStates() {
        try {
            ...
            const prefix = `reports_timer_${this.player.username}_`;
            ...
            console.log('[ReportsQuiz] Cleared all timer states');
        } catch (error) {
            console.warn('[ReportsQuiz] Failed to clear timer states:', error);
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('[ReportsQuiz] DOM loaded, initializing quiz...');
    window.reportsQuiz = new ReportsQuiz();
}); 