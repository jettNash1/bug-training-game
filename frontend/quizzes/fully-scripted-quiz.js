...
import { fullyScriptedScenarios } from '../data/fullyScripted-scenarios.js';

export class FullyScriptedQuiz extends BaseQuiz {
    constructor() {
        console.log('[FullyScriptedQuiz] Initializing...');
        ...
        this.basicScenarios = fullyScriptedScenarios.basic;
        this.intermediateScenarios = fullyScriptedScenarios.intermediate;
        this.advancedScenarios = fullyScriptedScenarios.advanced;
    ...
    async startGameWhenReady() {
        console.log('[FullyScriptedQuiz] Waiting for timer settings to be loaded...');
        ...
        const checkTimerSettings = () => {
            attempts++;
            console.log(`[FullyScriptedQuiz] Check attempt ${attempts}, timePerQuestion: ${this.timePerQuestion}`);
            ...
            if (this.timePerQuestion !== undefined && this.timePerQuestion !== null) {
                console.log(`[FullyScriptedQuiz] Timer settings loaded: ${this.timePerQuestion}s per question`);
            ...
            if (attempts >= maxAttempts) {
                console.warn('[FullyScriptedQuiz] Timer settings not loaded after maximum attempts, using BaseQuiz value');
    ...
    async startGame() {
        ...
        console.log('[FullyScriptedQuiz] Starting game...');
        ...
        try {
            ...
            console.log(`[FullyScriptedQuiz] Progress loaded: ${hasProgress}`);
            ...
            if (!hasProgress) {
                ...
                if (!this.player.questionHistory || !Array.isArray(this.player.questionHistory)) {
                    console.log('[FullyScriptedQuiz] Invalid question history in loaded progress, resetting');
                    this.player.questionHistory = [];
                }
                
                // CRITICAL: Ensure currentScenario is set correctly based on question history
                this.player.currentScenario = this.player.questionHistory.length;
                console.log('[FullyScriptedQuiz] Set currentScenario to match question history:', this.player.currentScenario);
            }
            ...
            console.error('[FullyScriptedQuiz] Error starting game:', error);
            this.isLoading = false;
    ...
    initializeTimer() {
        console.log(`[FullyScriptedQuiz] Initializing timer for question ${this.player.questionHistory.length}`);
        ...
        if (!timerContainer || !timerDisplay) {
            console.error('[FullyScriptedQuiz] Timer elements not found');
            return;
        }
        
        // Check if timer is disabled (0 seconds) or timer functionality is disabled
        if (this.timerDisabled || this.timePerQuestion === 0) {
            console.log(`[FullyScriptedQuiz] Timer is disabled, hiding timer display`);
        ...
        if (restoredTime !== null) {
            ...
            console.log(`[FullyScriptedQuiz] Restored timer with ${timeLeft} seconds remaining`);
        } else {
            timeLeft = this.timePerQuestion;
            console.log(`[FullyScriptedQuiz] Starting new timer with ${timeLeft} seconds`);
        }
    ...
    handleTimedOut() {
        console.log('[FullyScriptedQuiz] Question timed out');
    ...
    displayScenario() {
        ...
        console.log(`[FullyScriptedQuiz] Displaying scenario #${this.player.currentScenario + 1}:`, {
        ...
        if (this.player.questionHistory.length > 0) {
            ...
            this.saveProgress('in-progress').catch(err => {
                console.warn('[FullyScriptedQuiz] Save after display failed:', err);
            });
        }
    }
    ...
    async handleAnswer(timedOut = false) {
        ...
        if (submitButton) {
            ...
            console.log('[FullyScriptedQuiz] Selected answer:', {
            ...
            console.error('[FullyScriptedQuiz] Error handling answer:', error);
    ...
    async endGame(failed = false) {
        console.log('[FullyScriptedQuiz] Ending game...');
        ...
        try {
            ...
            console.log('[FullyScriptedQuiz] Quiz results:', {
        ...
        if (reviewList) {
            ...
            console.error('[FullyScriptedQuiz] Error ending game:', error);
    ...
    // Restart the quiz
    async restartQuiz() {
        console.log('[FullyScriptedQuiz] Restarting quiz...');
    ...
    showError(message) {
        console.error('[FullyScriptedQuiz] Error:', message);
    ...
    getTimerStorageKey() {
        ...
        return `fully-scripted_timer_${username}_q${questionIndex}`;
    }
    ...
    saveTimerState(timeRemaining) {
        try {
            ...
            console.warn('[FullyScriptedQuiz] Failed to save timer state:', error);
        }
    }
    ...
    restoreTimerState() {
        try {
            ...
            if (timerData.questionIndex !== currentQuestionIndex) {
                console.log('[FullyScriptedQuiz] Timer data is for different question, ignoring');
            ...
            if (Date.now() - timerData.timestamp > maxAge) {
                console.log('[FullyScriptedQuiz] Timer data is too old, ignoring');
                localStorage.removeItem(storageKey);
                return null;
            }
            ...
            console.log(`[FullyScriptedQuiz] Restored timer state: ${timerData.timeRemaining}s remaining`);
            return timerData.timeRemaining;
            
        } catch (error) {
            console.warn('[FullyScriptedQuiz] Failed to restore timer state:', error);
            return null;
        }
    }
    ...
    clearCurrentTimerState() {
        try {
            const storageKey = this.getTimerStorageKey();
            localStorage.removeItem(storageKey);
            console.log('[FullyScriptedQuiz] Cleared current timer state');
        } catch (error) {
            console.warn('[FullyScriptedQuiz] Failed to clear timer state:', error);
        }
    }
    ...
    clearAllTimerStates() {
        try {
            const username = localStorage.getItem('username') || 'anonymous';
            const prefix = `fully-scripted_timer_${username}_`;
            ...
            console.log(`[FullyScriptedQuiz] Cleared ${keysToRemove.length} timer states`);
        } catch (error) {
            console.warn('[FullyScriptedQuiz] Failed to clear timer states:', error);
        }
    }
    ...
    saveCurrentTimerState() {
        ...
        try {
            ...
            console.error('[FullyScriptedQuiz] Error saving current timer state:', error);
        }
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('[FullyScriptedQuiz] DOM loaded, initializing quiz...');
    window.fullyScriptedQuiz = new FullyScriptedQuiz();
}); 