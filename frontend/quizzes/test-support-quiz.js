...
export class TestSupportQuiz extends BaseQuiz {
    ...
    async startGameWhenReady() {
        console.log('[TestSupportQuiz] Waiting for timer settings to be loaded...');
        ...
        const checkTimerSettings = () => {
            attempts++;
            console.log(`[TestSupportQuiz] Check attempt ${attempts}, timePerQuestion: ${this.timePerQuestion}`);
            ...
            if (this.timePerQuestion !== undefined && this.timePerQuestion !== null) {
                console.log(`[TestSupportQuiz] Timer settings loaded: ${this.timePerQuestion}s per question`);
            ...
            if (attempts >= maxAttempts) {
                console.warn('[TestSupportQuiz] Timer settings not loaded after maximum attempts, using BaseQuiz value');
    ...
    initializeTimer() {
        console.log(`[TestSupportQuiz] Initializing timer for question ${this.player.questionHistory.length}`);
        ...
        if (this.timerDisabled || this.timePerQuestion === 0) {
            console.log(`[TestSupportQuiz] Timer is disabled, hiding timer display`);
        ...
        if (restoredTime !== null) {
            ...
            console.log(`[TestSupportQuiz] Restored timer with ${timeLeft} seconds remaining`);
        } else {
            timeLeft = this.timePerQuestion;
            console.log(`[TestSupportQuiz] Starting new timer with ${timeLeft} seconds`);
        }
    ...
    async handleAnswer(timedOut = false) {
        ...
        if (submitButton) {
            ...
            this.saveProgress().catch(err => {
                console.error('[TestSupportQuiz] Failed to save progress:', err);
            });
    ...
    getTimerStorageKey() {
        ...
        return `test-support_timer_${username}_q${questionIndex}`;
    }
    ...
    saveTimerState(timeRemaining) {
        try {
            ...
            console.warn('[TestSupportQuiz] Failed to save timer state:', error);
        }
    }
    ...
    restoreTimerState() {
        try {
            ...
            if (timerData.questionIndex !== currentQuestionIndex) {
                console.log('[TestSupportQuiz] Timer data is for different question, ignoring');
            ...
            if (Date.now() - timerData.timestamp > maxAge) {
                console.log('[TestSupportQuiz] Timer data is too old, ignoring');
                localStorage.removeItem(storageKey);
                return null;
            }
            ...
            console.log(`[TestSupportQuiz] Restored timer state: ${timerData.timeRemaining}s remaining`);
            return timerData.timeRemaining;
            
        } catch (error) {
            console.warn('[TestSupportQuiz] Failed to restore timer state:', error);
            return null;
        }
    }
    ...
    clearCurrentTimerState() {
        try {
            const storageKey = this.getTimerStorageKey();
            localStorage.removeItem(storageKey);
            console.log('[TestSupportQuiz] Cleared current timer state');
        } catch (error) {
            console.warn('[TestSupportQuiz] Failed to clear timer state:', error);
        }
    }
    ...
    clearAllTimerStates() {
        try {
            const username = localStorage.getItem('username') || 'anonymous';
            const prefix = `test-support_timer_${username}_`;
            ...
            console.log(`[TestSupportQuiz] Cleared ${keysToRemove.length} timer states`);
        } catch (error) {
            console.warn('[TestSupportQuiz] Failed to clear timer states:', error);
        }
    }
    ...
    saveCurrentTimerState() {
        ...
        try {
            ...
            console.error('[TestSupportQuiz] Error saving current timer state:', error);
        }
    }
}
