...
export class ContentCopyQuiz extends BaseQuiz {
    ...
    async startGameWhenReady() {
        console.log('[ContentCopyQuiz] Waiting for timer settings to be loaded...');
        ...
        while (attempts < maxAttempts) {
            if (this.timePerQuestion !== undefined && this.timePerQuestion !== null) {
                console.log('[ContentCopyQuiz] Timer settings ready, starting game...');
                break;
            }
        ...
        if (attempts >= maxAttempts) {
            console.warn('[ContentCopyQuiz] Timeout waiting for timer settings, proceeding with defaults');
        }
    ...
    initializeTimer() {
        console.log(`[ContentCopyQuiz] Initializing timer for question ${this.player.questionHistory.length}`);
        ...
        if (this.timerDisabled || this.timePerQuestion === 0) {
            console.log(`[ContentCopyQuiz] Timer is disabled, hiding timer display`);
        ...
        if (restoredTime !== null) {
            ...
            console.log(`[ContentCopyQuiz] Restored timer with ${timeLeft} seconds remaining`);
        } else {
            timeLeft = this.timePerQuestion;
            console.log(`[ContentCopyQuiz] Starting new timer with ${timeLeft} seconds`);
        }
    ...
    async handleAnswer(timedOut = false) {
        ...
        if (submitButton) {
            ...
            this.saveProgress().catch(err => {
                console.error('[ContentCopyQuiz] Failed to save progress:', err);
            });
    ...
    getTimerStorageKey() {
        ...
        return `content-copy_timer_${username}_q${this.player.questionHistory.length}`;
    }
    ...
    saveTimerState(timeRemaining) {
        try {
            ...
            console.error('[ContentCopyQuiz] Error saving timer state:', error);
        }
    }
    ...
    restoreTimerState() {
        try {
            ...
            if (!timerDataStr) {
                console.log('[ContentCopyQuiz] No timer state found to restore');
                return null;
            }
            ...
            if (!timerData || 
                ...
                console.warn('[ContentCopyQuiz] Invalid timer state data, removing');
            ...
            if (timerData.questionIndex !== this.player.questionHistory.length) {
                console.log(`[ContentCopyQuiz] Timer state is for question ${timerData.questionIndex}, but current is ${this.player.questionHistory.length}. Clearing old state.`);
                localStorage.removeItem(storageKey);
                return null;
            }
            ...
            if (timeDiff > 10 * 60 * 1000) { // 10 minutes
                console.log('[ContentCopyQuiz] Timer state is too old, removing');
                localStorage.removeItem(storageKey);
                return null;
            }
            ...
            if (timerData.timeRemaining <= 0 || timerData.timeRemaining > this.timePerQuestion) {
                console.log('[ContentCopyQuiz] Invalid time remaining in stored state, removing');
                localStorage.removeItem(storageKey);
                return null;
            }
            
            console.log(`[ContentCopyQuiz] Restored timer state: ${timerData.timeRemaining}s remaining for question ${timerData.questionIndex}`);
            return timerData.timeRemaining;
        } catch (error) {
            console.error('[ContentCopyQuiz] Error restoring timer state:', error);
            return null;
        }
    }
    ...
    clearCurrentTimerState() {
        try {
            const storageKey = this.getTimerStorageKey();
            localStorage.removeItem(storageKey);
            console.log(`[ContentCopyQuiz] Cleared timer state for question ${this.player.questionHistory.length}`);
        } catch (error) {
            console.error('[ContentCopyQuiz] Error clearing timer state:', error);
        }
    }
    ...
    clearAllTimerStates() {
        try {
            const username = localStorage.getItem('username') || 'anonymous';
            const prefix = `content-copy_timer_${username}_`;
            ...
            console.log(`[ContentCopyQuiz] Cleared ${keysToRemove.length} timer states`);
        } catch (error) {
            console.error('[ContentCopyQuiz] Error clearing all timer states:', error);
        }
    }
    ...
    saveCurrentTimerState() {
        ...
        try {
            ...
            console.error('[ContentCopyQuiz] Error saving current timer state:', error);
        }
    }
}
