...
export class RaisingTicketsQuiz extends BaseQuiz {
    ...
    async startGameWhenReady() {
        console.log('[RaisingTicketsQuiz] Waiting for timer settings to be loaded...');
        ...
        const checkTimerSettings = () => {
            attempts++;
            console.log(`[RaisingTicketsQuiz] Check attempt ${attempts}, timePerQuestion: ${this.timePerQuestion}`);
            ...
            if (this.timePerQuestion !== undefined && this.timePerQuestion !== null) {
                console.log(`[RaisingTicketsQuiz] Timer settings loaded: ${this.timePerQuestion}s per question`);
            ...
            if (attempts >= maxAttempts) {
                console.warn('[RaisingTicketsQuiz] Timer settings not loaded after maximum attempts, using BaseQuiz value');
    ...
    initializeTimer() {
        console.log(`[RaisingTicketsQuiz] Initializing timer for question ${this.player.questionHistory.length}`);
        ...
        if (this.timerDisabled || this.timePerQuestion === 0) {
            console.log(`[RaisingTicketsQuiz] Timer is disabled, hiding timer display`);
        ...
        if (restoredTime !== null) {
            ...
            console.log(`[RaisingTicketsQuiz] Restored timer with ${timeLeft} seconds remaining`);
        } else {
            timeLeft = this.timePerQuestion;
            console.log(`[RaisingTicketsQuiz] Starting new timer with ${timeLeft} seconds`);
        }
    ...
    async handleAnswer(timedOut = false) {
        ...
        if (submitButton) {
            ...
            this.saveProgress().catch(err => {
                console.error('[RaisingTicketsQuiz] Failed to save progress:', err);
            });
    ...
    saveTimerState(timeRemaining) {
        try {
            ...
            console.warn('[RaisingTicketsQuiz] Failed to save timer state:', error);
        }
    }
    ...
    restoreTimerState() {
        try {
            ...
            if (timerData.questionIndex !== currentQuestionIndex) {
                console.log('[RaisingTicketsQuiz] Timer data is for different question, ignoring');
            ...
            if (Date.now() - timerData.timestamp > maxAge) {
                console.log('[RaisingTicketsQuiz] Timer data is too old, ignoring');
                localStorage.removeItem(storageKey);
                return null;
            }
            ...
            console.log(`[RaisingTicketsQuiz] Restored timer state: ${timerData.timeRemaining}s remaining`);
            return timerData.timeRemaining;
            
        } catch (error) {
            console.warn('[RaisingTicketsQuiz] Failed to restore timer state:', error);
            return null;
        }
    }
    ...
    clearCurrentTimerState() {
        try {
            const storageKey = this.getTimerStorageKey();
            localStorage.removeItem(storageKey);
            console.log('[RaisingTicketsQuiz] Cleared current timer state');
        } catch (error) {
            console.warn('[RaisingTicketsQuiz] Failed to clear timer state:', error);
        }
    }
    ...
    saveCurrentTimerState() {
        ...
        try {
            ...
            console.error('[RaisingTicketsQuiz] Error saving current timer state:', error);
        }
    }
}
