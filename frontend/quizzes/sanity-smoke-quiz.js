...
export class SanitySmokeQuiz extends BaseQuiz {
    constructor() {
        console.log('[SanitySmokeQuiz] Initializing...');
        ...
        const config = {
            ...
            performanceThresholds: [
                { threshold: 90, message: '🏆 Outstanding! You\'re a sanity smoke testing expert!' },
                { threshold: 80, message: '👏 Great job! You\'ve shown strong sanity smoke testing instincts!' },
        ...
        this.basicScenarios = sanitySmokeScenarios.basic;
        this.intermediateScenarios = sanitySmokeScenarios.intermediate;
        this.advancedScenarios = sanitySmokeScenarios.advanced;
    ...
    async startGameWhenReady() {
        console.log('[SanitySmokeQuiz] Waiting for timer settings to be loaded...');
        ...
        const checkTimerSettings = () => {
            attempts++;
            console.log(`[SanitySmokeQuiz] Check attempt ${attempts}, timePerQuestion: ${this.timePerQuestion}`);
            ...
            if (this.timePerQuestion !== undefined && this.timePerQuestion !== null) {
                console.log(`[SanitySmokeQuiz] Timer settings loaded: ${this.timePerQuestion}s per question`);
            ...
            if (attempts >= maxAttempts) {
                console.warn('[SanitySmokeQuiz] Timer settings not loaded after maximum attempts, using BaseQuiz value');
    ...
    async startGame() {
        ...
        console.log('[SanitySmokeQuiz] Starting game...');
        ...
        try {
            ...
            console.log(`[SanitySmokeQuiz] Progress loaded: ${hasProgress}`);
            ...
            if (!hasProgress) {
                ...
                console.log('[SanitySmokeQuiz] No previous progress, starting fresh');
            } else {
                ...
                if (!this.player.questionHistory || !Array.isArray(this.player.questionHistory)) {
                    console.log('[SanitySmokeQuiz] Invalid question history in loaded progress, resetting');
                    this.player.questionHistory = [];
                }
                
                // CRITICAL: Ensure currentScenario is set correctly based on question history
                this.player.currentScenario = this.player.questionHistory.length;
                console.log('[SanitySmokeQuiz] Set currentScenario to match question history:', this.player.currentScenario);
            }
            ...
            console.error('[SanitySmokeQuiz] Error starting game:', error);
            this.isLoading = false;
    ...
    initializeTimer() {
        console.log(`[SanitySmoke] Initializing timer for question ${this.player.questionHistory.length}`);
        ...
        if (restoredTime !== null) {
            ...
            console.log(`[SanitySmoke] Restored timer with ${timeLeft} seconds remaining`);
        } else {
            timeLeft = this.timePerQuestion;
            console.log(`[SanitySmoke] Starting new timer with ${timeLeft} seconds`);
        }
    ...
    handleTimedOut() {
        console.log('[SanitySmokeQuiz] Question timed out');
    ...
    displayScenario() {
        ...
        console.log(`[SanitySmokeQuiz] Displaying scenario #${this.player.currentScenario + 1}:`, {
        ...
        if (this.player.questionHistory.length > 0) {
            ...
            this.saveProgress('in-progress').catch(err => {
                console.warn('[SanitySmokeQuiz] Save after display failed:', err);
            });
        }
    }
    ...
    async handleAnswer(timedOut = false) {
        ...
        if (submitButton) {
            ...
            console.log('[SanitySmokeQuiz] Selected answer:', {
            ...
            console.error('[SanitySmokeQuiz] Error handling answer:', error);
    ...
    async endGame(failed = false) {
        console.log('[SanitySmokeQuiz] Ending game...');
        ...
        try {
            ...
            console.log('[SanitySmokeQuiz] Quiz results:', {
        ...
        if (reviewList) {
            ...
            if (recommendations) {
                ...
                if (scorePercentage >= 90) {
                    recommendationsHTML = '<p>🌟 Outstanding! You have demonstrated excellent knowledge of sanity and smoke testing principles!</p>';
                    ...
                    incorrectQuestions.forEach(q => {
                        ...
                        if (q.scenario.level === 'Basic') {
                            recommendation = 'Review basic concepts of ';
                        } else if (q.scenario.level === 'Intermediate') {
                            recommendation = 'Study the relationship between ';
                        } else if (q.scenario.level === 'Advanced') {
                            recommendation = 'Deep dive into advanced topics of ';
                        }
                        recommendationsHTML += `<li>${recommendation}${q.scenario.title}: ${q.scenario.description}</li>`;
                    });
                    recommendationsHTML += '</ul>';
                } else {
                    recommendationsHTML = '<p>📚 Focus on these fundamental areas:</p><ul>';
                    // Find areas where the user made mistakes
                    const incorrectQuestions = this.player.questionHistory.filter(q => !q.isCorrect);
                    incorrectQuestions.forEach(q => {
                        // Add specific recommendations based on the scenario
                        let recommendation = '';
                        if (q.scenario.level === 'Basic') {
                            recommendation = 'Master the fundamentals of ';
                        } else if (q.scenario.level === 'Intermediate') {
                            recommendation = 'Understand the key concepts of ';
                        } else if (q.scenario.level === 'Advanced') {
                            recommendation = 'Build knowledge in advanced areas of ';
                        }
                        recommendationsHTML += `<li>${recommendation}${q.scenario.title}: ${q.scenario.description}</li>`;
                    });
                    recommendationsHTML += '</ul>';
            ...
            console.error('[SanitySmokeQuiz] Error ending game:', error);
    ...
    // Restart the quiz
    async restartQuiz() {
        console.log('[SanitySmokeQuiz] Restarting quiz...');
    ...
    showError(message) {
        console.error('[SanitySmokeQuiz] Error:', message);
    ...
    saveTimerState(timeRemaining) {
        try {
            ...
            console.warn('[SanitySmokeQuiz] Failed to save timer state:', error);
        }
    }
    ...
    restoreTimerState() {
        try {
            ...
            if (timerData.questionIndex !== currentQuestionIndex) {
                console.log('[SanitySmokeQuiz] Timer data is for different question, ignoring');
            ...
            if (Date.now() - timerData.timestamp > maxAge) {
                console.log('[SanitySmokeQuiz] Timer data is too old, ignoring');
                localStorage.removeItem(storageKey);
                return null;
            }
            ...
            console.log(`[SanitySmokeQuiz] Restored timer state: ${timerData.timeRemaining}s remaining`);
            return timerData.timeRemaining;
            
        } catch (error) {
            console.warn('[SanitySmokeQuiz] Failed to restore timer state:', error);
            return null;
        }
    }
    ...
    clearCurrentTimerState() {
        try {
            const storageKey = this.getTimerStorageKey();
            localStorage.removeItem(storageKey);
            console.log('[SanitySmokeQuiz] Cleared current timer state');
        } catch (error) {
            console.warn('[SanitySmokeQuiz] Failed to clear timer state:', error);
        }
    }
    ...
    saveCurrentTimerState() {
        ...
        try {
            ...
            console.error('[SanitySmokeQuiz] Error saving current timer state:', error);
        }
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('[SanitySmokeQuiz] DOM loaded, initializing quiz...');
    window.sanitySmokeQuiz = new SanitySmokeQuiz();
}); 