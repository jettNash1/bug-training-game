...
export class CMSTestingQuiz extends BaseQuiz {
    constructor() {
        console.log('[CMSTestingQuiz] Initializing...');
        ...
        this.basicScenarios = CMSTestingScenarios.basic;
        this.intermediateScenarios = CMSTestingScenarios.intermediate;
        this.advancedScenarios = CMSTestingScenarios.advanced;
    ...
    async startGame() {
        ...
        console.log('[CMSTestingQuiz] Starting game...');
        ...
        try {
            ...
            console.log(`[CMSTestingQuiz] Progress loaded: ${hasProgress}`);
            ...
            if (!hasProgress) {
                ...
                console.log('[CMSTestingQuiz] No previous progress, starting fresh');
            } else {
                ...
                if (!this.player.questionHistory || !Array.isArray(this.player.questionHistory)) {
                    console.log('[CMSTestingQuiz] Invalid question history in loaded progress, resetting');
                    this.player.questionHistory = [];
                }
                
                // CRITICAL: Ensure currentScenario is set correctly based on question history
                this.player.currentScenario = this.player.questionHistory.length;
                console.log('[CMSTestingQuiz] Set currentScenario to match question history:', this.player.currentScenario);
            }
            ...
            console.error('[CMSTestingQuiz] Error starting game:', error);
            this.isLoading = false;
    ...
    initializeTimer() {
        ...
        if (!timerContainer || !timerDisplay) {
            console.error('[CMSTestingQuiz] Timer elements not found');
            return;
        }
    ...
    handleTimedOut() {
        console.log('[CMSTestingQuiz] Question timed out');
    ...
    displayScenario() {
        ...
        console.log(`[CMSTestingQuiz] Displaying scenario #${this.player.currentScenario + 1}:`, {
        ...
        if (this.player.questionHistory.length > 0) {
            ...
            this.saveProgress('in-progress').catch(err => {
                console.warn('[CMSTestingQuiz] Save after display failed:', err);
            });
        }
    }
    ...
    async handleAnswer(timedOut = false) {
        ...
        if (submitButton) {
            ...
            console.log('[CMSTestingQuiz] Selected answer:', {
            ...
            console.error('[CMSTestingQuiz] Error handling answer:', error);
    ...
    async endGame(failed = false) {
        console.log('[CMSTestingQuiz] Ending game...');
        ...
        try {
            ...
            console.log('[CMSTestingQuiz] Quiz results:', {
        ...
        if (reviewList) {
            ...
            if (recommendations) {
                ...
                if (scorePercentage >= 90) {
                    recommendationsHTML = '<p>🌟 Outstanding! You have demonstrated excellent knowledge of CMS testing principles!</p>';
            ...
            console.error('[CMSTestingQuiz] Error ending game:', error);
    ...
    // Restart the quiz
    async restartQuiz() {
        console.log('[CMSTestingQuiz] Restarting quiz...');
    ...
    showError(message) {
        console.error('[CMSTestingQuiz] Error:', message);
        ...
        try {
            ...
            if (container) {
                ...
                console.error('[CMSTestingQuiz] Displayed error to user:', message);
            }
        } catch (e) {
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('[CMSTestingQuiz] DOM loaded, initializing quiz...');
    window.CMSTestingQuiz = new CMSTestingQuiz();
}); 