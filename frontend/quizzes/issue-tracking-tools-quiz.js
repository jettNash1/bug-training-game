...
export class IssueTrackingToolsQuiz extends BaseQuiz {
    constructor() {
        console.log('[IssueTrackingToolsQuiz] Initializing...');
        ...
        const config = {
            ...
            performanceThresholds: [
                { threshold: 90, message: '🏆 Outstanding! You\'re an issue tracking tools expert!' },
                { threshold: 80, message: '👏 Great job! You\'ve shown strong issue tracking tools instincts!' },
    ...
    async startGameWhenReady() {
        console.log('[IssueTrackingToolsQuiz] Waiting for timer settings to be loaded...');
        ...
        const checkTimerSettings = () => {
            attempts++;
            console.log(`[IssueTrackingToolsQuiz] Check attempt ${attempts}, timePerQuestion: ${this.timePerQuestion}`);
            ...
            if (this.timePerQuestion !== undefined && this.timePerQuestion !== null) {
                console.log(`[IssueTrackingToolsQuiz] Timer settings loaded: ${this.timePerQuestion}s per question`);
            ...
            if (attempts >= maxAttempts) {
                console.warn('[IssueTrackingToolsQuiz] Timer settings not loaded after maximum attempts, using BaseQuiz value');
    ...
    async startGame() {
        ...
        console.log('[IssueTrackingToolsQuiz] Starting game...');
        ...
        try {
            ...
            console.log(`[IssueTrackingToolsQuiz] Progress loaded: ${hasProgress}`);
            ...
            if (!hasProgress) {
                ...
                if (!this.player.questionHistory || !Array.isArray(this.player.questionHistory)) {
                    console.log('[IssueTrackingToolsQuiz] Invalid question history in loaded progress, resetting');
                    this.player.questionHistory = [];
                }
                
                // CRITICAL: Ensure currentScenario is set correctly based on question history
                this.player.currentScenario = this.player.questionHistory.length;
                console.log('[IssueTrackingToolsQuiz] Set currentScenario to match question history:', this.player.currentScenario);
            }
            ...
            console.error('[IssueTrackingToolsQuiz] Error starting game:', error);
            this.isLoading = false;
    ...
    initializeTimer() {
        console.log(`[IssueTrackingToolsQuiz] Initializing timer for question ${this.player.questionHistory.length}`);
        ...
        if (!timerContainer || !timerDisplay) {
            console.error('[IssueTrackingToolsQuiz] Timer elements not found');
            return;
        }
        
        // Check if timer is disabled (0 seconds) or timer functionality is disabled
        if (this.timerDisabled || this.timePerQuestion === 0) {
            console.log(`[IssueTrackingToolsQuiz] Timer is disabled, hiding timer display`);
        ...
        if (restoredTime !== null) {
            ...
            console.log(`[IssueTrackingToolsQuiz] Restored timer with ${timeLeft} seconds remaining`);
        } else {
            timeLeft = this.timePerQuestion;
            console.log(`[IssueTrackingToolsQuiz] Starting new timer with ${timeLeft} seconds`);
        }
    ...
    handleTimedOut() {
        console.log('[IssueTrackingToolsQuiz] Question timed out');
    ...
    displayScenario() {
        ...
        console.log(`[IssueTrackingToolsQuiz] Displaying scenario #${this.player.currentScenario + 1}:`, {
        ...
        if (this.player.questionHistory.length > 0) {
            ...
            this.saveProgress('in-progress').catch(err => {
                console.warn('[IssueTrackingToolsQuiz] Save after display failed:', err);
            });
        }
    }
    ...
    async handleAnswer(timedOut = false) {
        ...
        if (submitButton) {
            ...
            console.log('[IssueTrackingToolsQuiz] Selected answer:', {
            ...
            console.error('[IssueTrackingToolsQuiz] Error handling answer:', error);
    ...
    async endGame(failed = false) {
        console.log('[IssueTrackingToolsQuiz] Ending game...');
        ...
        try {
            ...
            console.log('[IssueTrackingToolsQuiz] Quiz results:', {
        ...
        if (reviewList) {
            ...
            if (recommendations) {
                ...
                if (scorePercentage >= 90) {
                    recommendationsHTML = '<p>🌟 Outstanding! You have demonstrated excellent knowledge of issue tracking tools and best practices!</p>';
            ...
            console.error('[IssueTrackingToolsQuiz] Error ending game:', error);
    ...
    // Restart the quiz
    async restartQuiz() {
        console.log('[IssueTrackingToolsQuiz] Restarting quiz...');
    ...
    showError(message) {
        console.error('[IssueTrackingToolsQuiz] Error:', message);
    ...
    getTimerStorageKey() {
        ...
        return `issue-tracking-tools_timer_${username}_q${questionIndex}`;
    }
    ...
    saveTimerState(timeRemaining) {
        try {
            ...
            console.warn('[IssueTrackingToolsQuiz] Failed to save timer state:', error);
        }
    }
    ...
    restoreTimerState() {
        try {
            ...
            if (timerData.questionIndex !== currentQuestionIndex) {
                console.log('[IssueTrackingToolsQuiz] Timer data is for different question, ignoring');
            ...
            if (Date.now() - timerData.timestamp > maxAge) {
                console.log('[IssueTrackingToolsQuiz] Timer data is too old, ignoring');
                localStorage.removeItem(storageKey);
                return null;
            }
            ...
            console.log(`[IssueTrackingToolsQuiz] Restored timer state: ${timerData.timeRemaining}s remaining`);
            return timerData.timeRemaining;
            
        } catch (error) {
            console.warn('[IssueTrackingToolsQuiz] Failed to restore timer state:', error);
            return null;
        }
    }
    ...
    clearCurrentTimerState() {
        try {
            const storageKey = this.getTimerStorageKey();
            localStorage.removeItem(storageKey);
            console.log('[IssueTrackingToolsQuiz] Cleared current timer state');
        } catch (error) {
            console.warn('[IssueTrackingToolsQuiz] Failed to clear timer state:', error);
        }
    }
    ...
    clearAllTimerStates() {
        try {
            const username = localStorage.getItem('username') || 'anonymous';
            const prefix = `issue-tracking-tools_timer_${username}_`;
            ...
            console.log(`[IssueTrackingToolsQuiz] Cleared ${keysToRemove.length} timer states`);
        } catch (error) {
            console.warn('[IssueTrackingToolsQuiz] Failed to clear timer states:', error);
        }
    }
    ...
    saveCurrentTimerState() {
        ...
        try {
            ...
            console.error('[IssueTrackingToolsQuiz] Error saving current timer state:', error);
        }
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('[IssueTrackingToolsQuiz] DOM loaded, initializing quiz...');
    window.issueTrackingToolsQuiz = new IssueTrackingToolsQuiz();
}); 