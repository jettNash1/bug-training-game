...
import { issueVerificationScenarios } from '../data/issueVerification-scenarios.js';

export class IssueVerificationQuiz extends BaseQuiz {
    constructor() {
        console.log('[IssueVerificationQuiz] Initializing...');
        ...
        const config = {
            ...
            performanceThresholds: [
                ...
                { threshold: 80, message: '👏 Great job! You\'ve shown strong issue verification instincts!' },
        ...
        this.basicScenarios = issueVerificationScenarios.basic;
        this.intermediateScenarios = issueVerificationScenarios.intermediate;
        this.advancedScenarios = issueVerificationScenarios.advanced;
    ...
    async startGameWhenReady() {
        ...
        const checkTimerSettings = () => {
            ...
            if (attempts >= maxAttempts) {
                console.warn('[IssueVerificationQuiz] Timer settings not loaded after maximum attempts, using BaseQuiz value');
    ...
    async startGame() {
        ...
        try {
            ...
            console.log(`[IssueVerificationQuiz] Progress loaded: ${hasProgress}`);
            ...
            if (!hasProgress) {
                ...
                console.log('[IssueVerificationQuiz] No previous progress, starting fresh');
            } else {
                ...
                if (!this.player.questionHistory || !Array.isArray(this.player.questionHistory)) {
                    console.log('[IssueVerificationQuiz] Invalid question history in loaded progress, resetting');
                    this.player.questionHistory = [];
                }
                
                // CRITICAL: Ensure currentScenario is set correctly based on question history
                this.player.currentScenario = this.player.questionHistory.length;
                console.log('[IssueVerificationQuiz] Set currentScenario to match question history:', this.player.currentScenario);
            }
    ...
    initializeTimer() {
        console.log(`[IssueVerificationQuiz] Initializing timer for question ${this.player.questionHistory.length}`);
        ...
        if (!timerContainer || !timerDisplay) {
            console.error('[IssueVerificationQuiz] Timer elements not found');
            return;
        }
        
        // Check if timer is disabled (0 seconds) or timer functionality is disabled
        if (this.timerDisabled || this.timePerQuestion === 0) {
            console.log(`[IssueVerificationQuiz] Timer is disabled, hiding timer display`);
        ...
        if (restoredTime !== null) {
            ...
            console.log(`[IssueVerificationQuiz] Restored timer with ${timeLeft} seconds remaining`);
        } else {
            timeLeft = this.timePerQuestion;
            console.log(`[IssueVerificationQuiz] Starting new timer with ${timeLeft} seconds`);
        }
    ...
    handleTimedOut() {
        console.log('[IssueVerificationQuiz] Question timed out');
    ...
    displayScenario() {
        ...
        console.log(`[IssueVerificationQuiz] Displaying scenario #${this.player.currentScenario + 1}:`, {
        ...
        if (this.player.questionHistory.length > 0) {
            ...
            this.saveProgress('in-progress').catch(err => {
                console.warn('[IssueVerificationQuiz] Save after display failed:', err);
            });
        }
    }
    ...
    async handleAnswer(timedOut = false) {
        ...
        if (submitButton) {
            ...
            console.log('[IssueVerificationQuiz] Selected answer:', {
            ...
            console.error('[IssueVerificationQuiz] Error handling answer:', error);
    ...
    async endGame(failed = false) {
        console.log('[IssueVerificationQuiz] Ending game...');
        ...
        try {
            ...
            console.log('[IssueVerificationQuiz] Quiz results:', {
        ...
        if (reviewList) {
            ...
            if (recommendations) {
                ...
                if (scorePercentage >= 90) {
                    ...
                    incorrectQuestions.forEach(q => {
                        ...
                        const tool = q.selectedAnswer.tool ? ` (${q.selectedAnswer.tool})` : '';
                        recommendationsHTML += `<li>Review ${q.scenario.title}${tool}: ${q.selectedAnswer.outcome}</li>`;
                    });
            ...
            console.error('[IssueVerificationQuiz] Error ending game:', error);
    ...
    // Restart the quiz
    async restartQuiz() {
        console.log('[IssueVerificationQuiz] Restarting quiz...');
    ...
    getTimerStorageKey() {
        ...
        return `issue-verification_timer_${username}_q${questionIndex}`;
    }
    ...
    clearAllTimerStates() {
        try {
            const username = localStorage.getItem('username') || 'anonymous';
            const prefix = `issue-verification_timer_${username}_`;
            ...
            console.log(`[IssueVerificationQuiz] Cleared ${keysToRemove.length} timer states`);
        } catch (error) {
    ...
    saveCurrentTimerState() {
        ...
        try {
            ...
            console.error('[IssueVerificationQuiz] Error saving current timer state:', error);
        }
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('[IssueVerificationQuiz] DOM loaded, initializing quiz...');
    window.issueVerificationQuiz = new IssueVerificationQuiz();
}); 