...
export class TestTypesTricksQuiz extends BaseQuiz {
        ...
        const checkTimerSettings = () => {
            ...
            if (this.timePerQuestion !== undefined && this.timePerQuestion !== null) {
                ...
                if (restoredTime !== null) {
                    ...
                    console.log(`[TestTypesTricksQuiz] Will restore timer to ${restoredTime}s`);
                }
            ...
            if (attempts >= maxAttempts) {
                console.warn('[TestTypesTricksQuiz] Timer settings not loaded after maximum attempts, using default');
    ...
    initializeTimer() {
        ...
        if (this.persistedTimeRemaining !== null) {
            ...
            console.log(`[TestTypesTricksQuiz] Resuming timer with ${timeLeft}s remaining`);
        } else {
            timeLeft = this.timePerQuestion;
            console.log(`[TestTypesTricksQuiz] Starting new timer with ${timeLeft}s`);
        }
        
        // If timer is 0 or disabled, hide timer and continue without timing
        if (timeLeft <= 0) {
            console.log('[TestTypesTricksQuiz] Timer disabled (0 seconds)');
