...
export class TestTypesTricksQuiz extends BaseQuiz {
    ...
    async handleAnswer(timedOut = false) {
        ...
        if (submitButton) {
            ...
            this.saveProgress().catch(err => {
                console.error('[TestTypesTricksQuiz] Failed to save progress:', err);
            });
    ...
    getTimerStorageKey() {
        ...
        return `test-types_timer_${username}_q${questionIndex}`;
    }
    ...
    clearAllTimerStates() {
        try {
            const username = localStorage.getItem('username') || 'anonymous';
            const prefix = `test-types_timer_${username}_`;
            ...
            console.log(`[TestTypesTricksQuiz] Cleared ${keysToRemove.length} timer states`);
        } catch (error) {
    ...
    saveCurrentTimerState() {
        ...
        try {
            ...
            console.error('[TestTypesTricksQuiz] Error saving current timer state:', error);
        }
    }
}
