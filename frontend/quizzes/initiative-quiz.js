...
export class InitiativeQuiz extends BaseQuiz {
    constructor() {
        console.log('[InitiativeQuiz] Initializing...');
        ...
        const config = {
            ...
            performanceThresholds: [
                ...
                { threshold: 80, message: '👏 Great job! You\'ve shown strong initiative instincts!' },
    ...
    async startGameWhenReady() {
        console.log('[InitiativeQuiz] Waiting for timer settings to be loaded...');
        ...
        const checkTimerSettings = () => {
            attempts++;
            console.log(`[InitiativeQuiz] Check attempt ${attempts}, timePerQuestion: ${this.timePerQuestion}`);
    ...
    async startGame() {
        ...
        console.log('[InitiativeQuiz] Starting game...');
        ...
        try {
            ...
            console.log(`[InitiativeQuiz] Progress loaded: ${hasProgress}`);
            ...
            if (!hasProgress) {
                ...
                console.log('[InitiativeQuiz] No previous progress, starting fresh');
            } else {
                ...
                if (!this.player.questionHistory || !Array.isArray(this.player.questionHistory)) {
                    console.log('[InitiativeQuiz] Invalid question history in loaded progress, resetting');
                    this.player.questionHistory = [];
                }
                
                // CRITICAL: Ensure currentScenario is set correctly based on question history
                this.player.currentScenario = this.player.questionHistory.length;
                console.log('[InitiativeQuiz] Set currentScenario to match question history:', this.player.currentScenario);
            }
            ...
            console.error('[InitiativeQuiz] Error starting game:', error);
            this.isLoading = false;
    ...
    initializeTimer() {
        ...
        if (!timerContainer || !timerDisplay) {
            console.error('[InitiativeQuiz] Timer elements not found');
            return;
        }
    ...
    handleTimedOut() {
        console.log('[InitiativeQuiz] Question timed out');
    ...
    displayScenario() {
        ...
        console.log(`[InitiativeQuiz] Displaying scenario #${this.player.currentScenario + 1}:`, {
        ...
        if (this.player.questionHistory.length > 0) {
            ...
            this.saveProgress('in-progress').catch(err => {
                console.warn('[InitiativeQuiz] Save after display failed:', err);
            });
        }
    }
    ...
    async handleAnswer(timedOut = false) {
        ...
        if (submitButton) {
            ...
            console.error('[InitiativeQuiz] Error handling answer:', error);
    ...
    async endGame(failed = false) {
        console.log('[InitiativeQuiz] Ending game...');
        ...
        try {
            ...
            console.log('[InitiativeQuiz] Quiz results:', {
        ...
        if (reviewList) {
            ...
            if (recommendations) {
                ...
                if (scorePercentage >= 90) {
                    recommendationsHTML = '<p>🌟 Outstanding! You have demonstrated excellent initiative and leadership skills!</p>';
                    ...
                    Object.entries(groupedByLevel).forEach(([level, questions]) => {
                        recommendationsHTML += `<li><strong>${level} Level Initiative Areas:</strong><ul>`;
                        questions.forEach(q => {
                            recommendationsHTML += `<li>${q.scenario.title}: Review how to show initiative in ${q.scenario.description.toLowerCase()}</li>`;
                        });
                        recommendationsHTML += '</ul></li>';
                    });
                    recommendationsHTML += '</ul>';
                } else {
                    recommendationsHTML = '<p>📚 Here are key areas to improve your initiative:</p><ul>';
                    // Find areas where the user made mistakes and group by level
                    const incorrectQuestions = this.player.questionHistory.filter(q => !q.isCorrect);
                    const groupedByLevel = incorrectQuestions.reduce((acc, q) => {
                        if (!acc[q.scenario.level]) {
                    ...
                    Object.entries(groupedByLevel).forEach(([level, questions]) => {
                        ...
                        questions.forEach(q => {
                            recommendationsHTML += `<li>${q.scenario.title}: ${q.scenario.description}<br>
                                <em>Remember: ${q.selectedAnswer.outcome}</em></li>`;
                        });
                        recommendationsHTML += '</ul></li>';
                    });
                    recommendationsHTML += '</ul><p>Focus on taking proactive action and identifying opportunities to add value without being asked.</p>';
                }
            ...
            console.error('[InitiativeQuiz] Error ending game:', error);
    ...
    // Restart the quiz
    async restartQuiz() {
        console.log('[InitiativeQuiz] Restarting quiz...');
    ...
    showError(message) {
        console.error('[InitiativeQuiz] Error:', message);
    ...
    getTimerStorageKey() {
        ...
        return `initiative_timer_${username}_q${questionIndex}`;
    }
    ...
    saveTimerState(timeRemaining) {
        try {
            ...
            console.warn('[InitiativeQuiz] Failed to save timer state:', error);
        }
    }
    ...
    restoreTimerState() {
        try {
            ...
            if (timerData.questionIndex !== currentQuestionIndex) {
                console.log('[InitiativeQuiz] Timer data is for different question, ignoring');
            ...
            if (Date.now() - timerData.timestamp > maxAge) {
                console.log('[InitiativeQuiz] Timer data is too old, ignoring');
                localStorage.removeItem(storageKey);
                return null;
            }
            ...
            console.log(`[InitiativeQuiz] Restored timer state: ${timerData.timeRemaining}s remaining`);
            return timerData.timeRemaining;
            
        } catch (error) {
            console.warn('[InitiativeQuiz] Failed to restore timer state:', error);
            return null;
        }
    }
    ...
    clearCurrentTimerState() {
        try {
            const storageKey = this.getTimerStorageKey();
            localStorage.removeItem(storageKey);
            console.log('[InitiativeQuiz] Cleared current timer state');
        } catch (error) {
            console.warn('[InitiativeQuiz] Failed to clear timer state:', error);
        }
    }
    ...
    clearAllTimerStates() {
        try {
            const username = localStorage.getItem('username') || 'anonymous';
            const prefix = `initiative_timer_${username}_`;
            ...
            console.log(`[InitiativeQuiz] Cleared ${keysToRemove.length} timer states`);
        } catch (error) {
            console.warn('[InitiativeQuiz] Failed to clear timer states:', error);
        }
    }
    ...
    saveCurrentTimerState() {
        ...
        try {
            ...
            console.error('[InitiativeQuiz] Error saving current timer state:', error);
        }
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('[InitiativeQuiz] DOM loaded, initializing quiz...');
    window.initiativeQuiz = new InitiativeQuiz();
}); 