...
import { initiativeScenarios } from '../data/initiative-scenarios.js';
...
export class InitiativeQuiz extends BaseQuiz {
    constructor() {
        ...
        this.basicScenarios = initiativeScenarios.basic;
        this.intermediateScenarios = initiativeScenarios.intermediate;
        this.advancedScenarios = initiativeScenarios.advanced;
        ...
        console.log('[InitiativeQuiz] Quiz name being used:', this.quizName);
        const relatedLocalStorage = Object.keys(localStorage).filter(k => 
            k.includes('quiz_progress') || k.includes('initiative')
        );
        console.log('[InitiativeQuiz] Related localStorage keys:', relatedLocalStorage);
    }
    ...
    showError(message) {
        try {
            ...
            if (container) {
                ...
                console.error('[InitiativeQuiz] Displayed error to user:', message);
            }
        } catch (e) {
            console.error('[InitiativeQuiz] Failed to show error to user:', e);
        }
    }
    ...
    async saveProgress() {
        ...
        try {
            ...
            try {
                ...
                console.log('[InitiativeQuiz] Backed up progress to sessionStorage');
                ...
                console.warn('[InitiativeQuiz] Failed to save to sessionStorage:', sessionError);
            }
            ...
            try {
                if (typeof quizSyncService !== 'undefined') {
                    ...
                    console.log('[InitiativeQuiz] Added to sync queue');
                } else {
                    throw new Error('Sync service not available');
                }
            } catch (syncError) {
                // Direct API saving as fallback
                console.warn('[InitiativeQuiz] Sync service failed, trying direct API save:', syncError);
                
                try {
                    await this.apiService.saveQuizProgress(this.quizName, progressData);
                    console.log('[InitiativeQuiz] Saved progress directly to API');
                } catch (apiError) {
                    console.error('[InitiativeQuiz] Failed to save to API:', apiError);
            ...
            console.error('[InitiativeQuiz] Failed to save progress:', error);
            ...
            try {
                ...
                console.log('[InitiativeQuiz] Saved emergency backup to sessionStorage');
            } catch (sessionError) {
                console.error('[InitiativeQuiz] All storage methods failed:', sessionError);
            }
    ...
    async loadProgress() {
        try {
            ...
            console.log('[InitiativeQuiz] ALL localStorage quiz progress keys for this user:', allStorageKeys);
            ...
            const storageKeys = [
                ...
                `quiz_progress_${username}_initiative`,  // Fallback for standardized name
                `quiz_progress_${username}_initiative_backup`,
                `quiz_progress_${username}_initiative_emergency`
            ];
            
            // Log which keys we will try
            console.log('[InitiativeQuiz] Will try these localStorage keys in order:', storageKeys);
            ...
            try {
                ...
                console.log('[InitiativeQuiz] SessionStorage keys found:', sessionKeys);
                ...
                if (sessionKeys.length > 0) {
                    ...
                    try {
                        ...
                        console.log('[InitiativeQuiz] Loaded most recent sessionStorage data from key:', 
                            mostRecentKey, 'with question count:', 
                            sessionStorageData.questionHistory?.length || 0);
                    } catch (e) {
                        console.warn('[InitiativeQuiz] Failed to parse session storage data:', e);
                    }
                }
            } catch (sessionError) {
                console.warn('[InitiativeQuiz] Error accessing sessionStorage:', sessionError);
            }
            
            // Try to get progress from API
            try {
                console.log('[InitiativeQuiz] Attempting to load progress from API');
                apiProgress = await this.apiService.getQuizProgress(this.quizName);
                console.log('[InitiativeQuiz] API progress response:', apiProgress);
                ...
                if (apiProgress && apiProgress.data) {
                    ...
                    if (apiHasProgress) {
                        console.log('[InitiativeQuiz] API data contains valid progress with questions:', 
                            apiProgress.data.questionHistory.length);
                    } else {
                        console.warn('[InitiativeQuiz] API returned data but without valid questions or progress');
                    }
                } else {
                    console.warn('[InitiativeQuiz] API returned no valid data');
                }
            } catch (apiError) {
                console.warn('[InitiativeQuiz] Failed to load progress from API:', apiError);
            }
                ...
                if (localData) {
                    try {
                        ...
                        if (candidateData && Array.isArray(candidateData.questionHistory)) {
                            ...
                            console.log(`[InitiativeQuiz] Found localStorage data in ${key} with ${questionCount} questions`);
                            ...
                            if (questionCount > bestQuestionCount) {
                                ...
                                console.log(`[InitiativeQuiz] This is now the best local storage data (${questionCount} questions)`);
                            }
                        }
                    } catch (parseError) {
                        console.error(`[InitiativeQuiz] Failed to parse localStorage data for key ${key}:`, parseError);
                    }
                }
            }
            ...
            if (apiProgress && apiProgress.data && 
                ...
                console.log(`[InitiativeQuiz] API data has ${bestCount} questions`);
            }
            ...
            if (bestLocalStorageData && 
                ...
                console.log(`[InitiativeQuiz] localStorage data has ${bestCount} questions, better than current best`);
            }
            ...
            if (sessionStorageData && 
                ...
                console.log(`[InitiativeQuiz] sessionStorage data has ${bestCount} questions, better than current best`);
            }
            
            // Use the best data we've found
            if (bestData) {
                console.log(`[InitiativeQuiz] Using best progress data from ${bestSource} with ${bestCount} questions`);
                ...
                if (bestSource !== 'API' && bestCount > 0) {
                    try {
                        console.log('[InitiativeQuiz] Syncing best progress data to API and localStorage');
                        ...
                        console.warn('[InitiativeQuiz] Failed to sync best progress data:', syncError);
                    }
                }
            } else {
                console.log('[InitiativeQuiz] No valid progress data found from any source, trying recovery...');
                ...
                try {
                    ...
                    if (typeof window.quizSyncService !== 'undefined' || typeof quizSyncService !== 'undefined') {
                        ...
                        if (recoveredData) {
                            console.log('[InitiativeQuiz] Successfully recovered data from QuizSyncService');
                            progressData = recoveredData;
                            dataSource = 'recovered';
                        } else {
                            console.warn('[InitiativeQuiz] No data could be recovered, returning false');
                            return false;
                        }
                    } else {
                        console.warn('[InitiativeQuiz] QuizSyncService not available, cannot recover');
                        return false;
                    }
                } catch (recoveryError) {
                    console.error('[InitiativeQuiz] Error during data recovery:', recoveryError);
                    return false;
                }
            }

            if (progressData) {
                console.log(`[InitiativeQuiz] Processing loaded progress data from ${dataSource}`);
                ...
                if (progressData.questionHistory.length > 0) {
                    console.log('[InitiativeQuiz] Setting currentScenario to match questionHistory.length:', 
                    ...
                    console.log('[InitiativeQuiz] No questions in history, starting from beginning');
                ...
                if ((progressData.status === 'completed' || 
                    ...
                    (progressData.questionHistory.length === 0 || 
                     progressData.currentScenario === 0)) {
                    console.log('[InitiativeQuiz] Fixing inconsistent state: quiz marked as completed but has no progress');
                ...
                console.log('[InitiativeQuiz] Player state updated:', {
                    experience: this.player.experience,
                ...
                if ((progressData.status === 'completed' || 
                    ...
                    progressData.questionHistory.length > 0 && 
                    progressData.currentScenario > 0) {
                    console.log(`[InitiativeQuiz] Quiz is ${progressData.status} with ${progressData.questionHistory.length} questions answered`);
                ...
                if (this.player.questionHistory.length === 0 && progressData.questionHistory.length > 0) {
                    console.error('[InitiativeQuiz] CRITICAL ERROR: Failed to load question history properly!');
                    // Forced retry with direct assignment
                    this.player.questionHistory = [...progressData.questionHistory];
                    this.player.currentScenario = this.player.questionHistory.length;
                    console.log('[InitiativeQuiz] Forced player state update after error:', {
                        questionHistory: this.player.questionHistory.length,
                        currentScenario: this.player.currentScenario
                    });
                }
            ...
            console.log('[InitiativeQuiz] No existing progress found');
            return false;
        } catch (error) {
            ...
            try {
                console.log('[InitiativeQuiz] Attempting emergency progress recovery after error');
                return await this.recoverProgress();
            } catch (recoveryError) {
                console.error('[InitiativeQuiz] Emergency recovery also failed:', recoveryError);
                return false;
            }
        }
    }
    ...
    async startGame() {
        ...
        try {
            ...
            console.log('[InitiativeQuiz] Progress status check:', {
    ...
    displayScenario() {
        ...
        console.log('[InitiativeQuiz][displayScenario] Showing scenario:', {
        ...
        if (currentScenarioIndex !== totalAnswered) {
            console.warn('[InitiativeQuiz][displayScenario] Misalignment detected! currentScenario and questionHistory.length don\'t match:',
            ...
            if (totalAnswered > 0 && currentScenarioIndex === 0) {
                console.log('[InitiativeQuiz][displayScenario] Auto-fixing: Setting currentScenario to match questionHistory.length');
        ...
        if (currentScenarioIndex >= totalQuestions) {
            console.log('[InitiativeQuiz][displayScenario] All questions answered, ending game');
            this.endGame(false);
            return;
        }
        ...
        try {
            ...
            if (!scenario) {
                console.error('[InitiativeQuiz][displayScenario] No scenario found for currentScenario:', currentScenarioIndex);
                console.log('[InitiativeQuiz][displayScenario] Scenario sets:', {
                ...
                if (totalAnswered > 0 && totalAnswered < totalQuestions) {
                    console.log('[InitiativeQuiz][displayScenario] Attempting recovery by moving to next available scenario');
                    this.player.currentScenario = totalAnswered;
                    ...
                    console.log('[InitiativeQuiz][displayScenario] Attempting emergency data recovery');
            ...
            if (!titleElement || !descriptionElement || !optionsContainer) {
                console.error('[InitiativeQuiz][displayScenario] Required elements not found');
                return;
            }
            ...
            console.log('[InitiativeQuiz][displayScenario] Showing scenario', scenarioLevel, 'index', currentScenarioIndex, scenario.title);

            // Make scenario screen visible if not already
            const gameScreen = document.getElementById('game-screen');
            if (gameScreen && gameScreen.classList.contains('hidden')) {
                console.log('[InitiativeQuiz][displayScenario] Making game screen visible');
                gameScreen.classList.remove('hidden');
            }

            // Add extra log after showing
            setTimeout(() => {
                console.log('[InitiativeQuiz][displayScenario][post-show] player state:', {
                    experience: this.player.experience,
                    questionHistory: this.player.questionHistory.length,
                    currentScenario: this.player.currentScenario
                });
            ...
            console.error('[InitiativeQuiz][displayScenario] Error showing scenario:', error);
            // Emergency recovery if an error occurs
            const gameScreen = document.getElementById('game-screen');
            if (gameScreen) gameScreen.classList.remove('hidden');
            
            // Try to recover progress data in case of display error
            console.log('[InitiativeQuiz][displayScenario] Attempting emergency recovery after display error');
            this.recoverProgress().then(success => {
                if (!success) {
    ...
    async loadScenariosWithCaching() {
        ...
        if (cachedData && cacheValid) {
            console.log('[InitiativeQuiz] Using cached scenarios');
            const data = JSON.parse(cachedData);
            this.basicScenarios = data.basic || initiativeScenarios.basic;
            this.intermediateScenarios = data.intermediate || initiativeScenarios.intermediate;
            this.advancedScenarios = data.advanced || initiativeScenarios.advanced;
            return;
        }
        
        // If no valid cache, try to fetch from API
        try {
            console.log('[InitiativeQuiz] Fetching scenarios from API');
            ...
            if (data && data.scenarios) {
                ...
                // Update scenarios
                this.basicScenarios = data.scenarios.basic || initiativeScenarios.basic;
                this.intermediateScenarios = data.scenarios.intermediate || initiativeScenarios.intermediate;
                this.advancedScenarios = data.scenarios.advanced || initiativeScenarios.advanced;
            }
        } catch (error) {
            console.error('[InitiativeQuiz] Failed to load scenarios from API:', error);
            console.log('[InitiativeQuiz] Falling back to default scenarios');
    ...
    async recoverProgress() {
        console.log('[InitiativeQuiz] Entering emergency recovery mode');
        try {
            const username = localStorage.getItem('username');
            if (!username) {
                console.error('[InitiativeQuiz] No username found, cannot recover progress');
                return false;
            }
            
            // Make sure we have access to the QuizSyncService
            if (typeof window.quizSyncService === 'undefined' && typeof quizSyncService === 'undefined') {
                console.error('[InitiativeQuiz] QuizSyncService not available, cannot perform recovery');
                return false;
            }
            
            const syncService = window.quizSyncService || quizSyncService;
            console.log('[InitiativeQuiz] Using QuizSyncService to recover progress data');
            
            // Try to recover progress data
            const recoveredData = await syncService.recoverProgressData(username, this.quizName);
            
            if (!recoveredData) {
                console.warn('[InitiativeQuiz] No data could be recovered, recovery failed');
                return false;
            }
            
            console.log('[InitiativeQuiz] Successfully recovered data:', {
            ...
            console.log('[InitiativeQuiz] Player state updated from recovered data:', {
                experience: this.player.experience,
                questionHistory: this.player.questionHistory.length,
                currentScenario: this.player.currentScenario
            });
            ...
            if ((progressData.status === 'completed' || 
                 ...
                 progressData.status === 'failed') && 
                progressData.questionHistory.length > 0) {
                console.log(`[InitiativeQuiz] Quiz is ${progressData.status}, showing end screen`);
                this.endGame(progressData.status === 'failed');
                return true;
            }
            ...
            try {
                await this.saveProgress();
                console.log('[InitiativeQuiz] Saved recovered progress to all storage locations');
            } catch (saveError) {
                console.warn('[InitiativeQuiz] Failed to save recovered progress:', saveError);
            }
            ...
            console.error('[InitiativeQuiz] Recovery attempt failed:', error);
            return false;
        }
    }
}

// Singleton instance for InitiativeQuiz
let initiativeQuizInstance = null;

// Initialize quiz when the page loads
// Only allow one instance
document.addEventListener('DOMContentLoaded', () => {
    if (initiativeQuizInstance) {
        console.log('[InitiativeQuiz] Instance already exists, not creating a new one.');
        return;
    }
    BaseQuiz.clearQuizInstances('initiative');
    initiativeQuizInstance = new InitiativeQuiz();
    initiativeQuizInstance.startGame();
}); 