...
export class TesterMindsetQuiz extends BaseQuiz {
    constructor() {
        console.log('[TesterMindsetQuiz] Initializing...');
    ...
    async startGameWhenReady() {
        console.log('[TesterMindsetQuiz] Waiting for timer settings to be loaded...');
        ...
        const checkTimerSettings = () => {
            attempts++;
            console.log(`[TesterMindsetQuiz] Check attempt ${attempts}, timePerQuestion: ${this.timePerQuestion}`);
            ...
            if (this.timePerQuestion !== undefined && this.timePerQuestion !== null) {
                console.log(`[TesterMindsetQuiz] Timer settings loaded: ${this.timePerQuestion}s per question`);
            ...
            if (attempts >= maxAttempts) {
                console.warn('[TesterMindsetQuiz] Timer settings not loaded after maximum attempts, using BaseQuiz value');
    ...
    async startGame() {
        ...
        console.log('[TesterMindsetQuiz] Starting game...');
        ...
        try {
            ...
            if (!hasProgress) {
                ...
                console.log('[TesterMindsetQuiz] No previous progress, starting fresh');
            } else {
                ...
                if (!this.player.questionHistory || !Array.isArray(this.player.questionHistory)) {
                    console.log('[TesterMindsetQuiz] Invalid question history in loaded progress, resetting');
                    this.player.questionHistory = [];
                }
                
                // CRITICAL: Ensure currentScenario is set correctly based on question history
                this.player.currentScenario = this.player.questionHistory.length;
                console.log('[TesterMindsetQuiz] Set currentScenario to match question history:', this.player.currentScenario);
            }
    ...
    initializeTimer() {
        console.log(`[TesterMindsetQuiz] Initializing timer for question ${this.player.questionHistory.length}`);
        ...
        if (!timerContainer || !timerDisplay) {
            console.error('[TesterMindsetQuiz] Timer elements not found');
            return;
        }
        
        // Check if timer is disabled (0 seconds) or timer functionality is disabled
        if (this.timerDisabled || this.timePerQuestion === 0) {
            console.log(`[TesterMindsetQuiz] Timer is disabled, hiding timer display`);
        ...
        if (restoredTime !== null) {
            ...
            console.log(`[TesterMindsetQuiz] Restored timer with ${timeLeft} seconds remaining`);
        } else {
            timeLeft = this.timePerQuestion;
            console.log(`[TesterMindsetQuiz] Starting new timer with ${timeLeft} seconds`);
        }
    ...
    handleTimedOut() {
        console.log('[TesterMindsetQuiz] Question timed out');
    ...
    displayScenario() {
        ...
        if (this.player.questionHistory.length > 0) {
            ...
            this.saveProgress('in-progress').catch(err => {
                console.warn('[TesterMindsetQuiz] Save after display failed:', err);
            });
        }
    }
    ...
    async handleAnswer(timedOut = false) {
        ...
        if (submitButton) {
            ...
            console.error('[TesterMindsetQuiz] Error handling answer:', error);
    ...
    async endGame(failed = false) {
        ...
        try {
            ...
            console.log('[TesterMindsetQuiz] Quiz results:', {
        ...
        if (reviewList) {
            ...
            console.error('[TesterMindsetQuiz] Error ending game:', error);
    ...
    // Restart the quiz
    async restartQuiz() {
        console.log('[TesterMindsetQuiz] Restarting quiz...');
    ...
    showError(message) {
        console.error('[TesterMindsetQuiz] Error:', message);
    ...
    getTimerStorageKey() {
        ...
        return `tester-mindset_timer_${username}_q${questionIndex}`;
    }
    ...
    saveTimerState(timeRemaining) {
        try {
            ...
            console.warn('[TesterMindsetQuiz] Failed to save timer state:', error);
        }
    }
    ...
    restoreTimerState() {
        try {
            ...
            if (timerData.questionIndex !== currentQuestionIndex) {
                console.log('[TesterMindsetQuiz] Timer data is for different question, ignoring');
            ...
            if (Date.now() - timerData.timestamp > maxAge) {
                console.log('[TesterMindsetQuiz] Timer data is too old, ignoring');
                localStorage.removeItem(storageKey);
                return null;
            }
            ...
            console.log(`[TesterMindsetQuiz] Restored timer state: ${timerData.timeRemaining}s remaining`);
            return timerData.timeRemaining;
            
        } catch (error) {
            console.warn('[TesterMindsetQuiz] Failed to restore timer state:', error);
            return null;
        }
    }
    ...
    clearCurrentTimerState() {
        try {
            const storageKey = this.getTimerStorageKey();
            localStorage.removeItem(storageKey);
            console.log('[TesterMindsetQuiz] Cleared current timer state');
        } catch (error) {
            console.warn('[TesterMindsetQuiz] Failed to clear timer state:', error);
        }
    }
    ...
    clearAllTimerStates() {
        try {
            const username = localStorage.getItem('username') || 'anonymous';
            const prefix = `tester-mindset_timer_${username}_`;
            ...
            console.log(`[TesterMindsetQuiz] Cleared ${keysToRemove.length} timer states`);
        } catch (error) {
            console.warn('[TesterMindsetQuiz] Failed to clear timer states:', error);
        }
    }
    ...
    saveCurrentTimerState() {
        ...
        try {
            ...
            console.error('[TesterMindsetQuiz] Error saving current timer state:', error);
        }
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('[TesterMindsetQuiz] DOM loaded, initializing quiz...');
    window.testerMindsetQuiz = new TesterMindsetQuiz();
}); 