...
import { newTicketTemplateScenarios } from '../data/newTicketTemplate-scenarios.js';

export class NewTicketTemplateQuiz extends BaseQuiz {
    constructor() {
        console.log('[NewTicketTemplateQuiz] Initializing...');
        ...
        const config = {
            ...
            performanceThresholds: [
                { threshold: 90, message: '🏆 Outstanding! You\'re a new ticket template expert!' },
                { threshold: 80, message: '👏 Great job! You\'ve shown strong new ticket template instincts!' },
        ...
        this.basicScenarios = newTicketTemplateScenarios.basic;
        this.intermediateScenarios = newTicketTemplateScenarios.intermediate;
        this.advancedScenarios = newTicketTemplateScenarios.advanced;
    ...
    async startGameWhenReady() {
        console.log('[NewTicketTemplateQuiz] Waiting for timer settings to be loaded...');
        ...
        const checkTimerSettings = () => {
            attempts++;
            console.log(`[NewTicketTemplateQuiz] Check attempt ${attempts}, timePerQuestion: ${this.timePerQuestion}`);
            ...
            if (this.timePerQuestion !== undefined && this.timePerQuestion !== null) {
                console.log(`[NewTicketTemplateQuiz] Timer settings loaded: ${this.timePerQuestion}s per question`);
            ...
            if (attempts >= maxAttempts) {
                console.warn('[NewTicketTemplateQuiz] Timer settings not loaded after maximum attempts, using BaseQuiz value');
    ...
    async startGame() {
        ...
        console.log('[NewTicketTemplateQuiz] Starting game...');
        ...
        try {
            ...
            console.log(`[NewTicketTemplateQuiz] Progress loaded: ${hasProgress}`);
            ...
            if (!hasProgress) {
                ...
                console.log('[NewTicketTemplateQuiz] No previous progress, starting fresh');
            } else {
                ...
                if (!this.player.questionHistory || !Array.isArray(this.player.questionHistory)) {
                    console.log('[NewTicketTemplateQuiz] Invalid question history in loaded progress, resetting');
                    this.player.questionHistory = [];
                }
                
                // CRITICAL: Ensure currentScenario is set correctly based on question history
                this.player.currentScenario = this.player.questionHistory.length;
                console.log('[NewTicketTemplateQuiz] Set currentScenario to match question history:', this.player.currentScenario);
            }
            ...
            console.error('[NewTicketTemplateQuiz] Error starting game:', error);
            this.isLoading = false;
    ...
    initializeTimer() {
        console.log(`[NewTicketTemplateQuiz] Initializing timer for question ${this.player.questionHistory.length}`);
        ...
        if (!timerContainer || !timerDisplay) {
            console.error('[NewTicketTemplateQuiz] Timer elements not found');
            return;
        }
        
        // Check if timer is disabled (0 seconds) or timer functionality is disabled
        if (this.timerDisabled || this.timePerQuestion === 0) {
            console.log(`[NewTicketTemplateQuiz] Timer is disabled, hiding timer display`);
        ...
        if (restoredTime !== null) {
            ...
            console.log(`[NewTicketTemplateQuiz] Restored timer with ${timeLeft} seconds remaining`);
        } else {
            timeLeft = this.timePerQuestion;
            console.log(`[NewTicketTemplateQuiz] Starting new timer with ${timeLeft} seconds`);
        }
    ...
    handleTimedOut() {
        console.log('[NewTicketTemplateQuiz] Question timed out');
    ...
    displayScenario() {
        ...
        if (this.player.questionHistory.length > 0) {
            ...
            this.saveProgress('in-progress').catch(err => {
                console.warn('[NewTicketTemplateQuiz] Save after display failed:', err);
            });
        }
    }
    ...
    async handleAnswer(timedOut = false) {
        ...
        if (submitButton) {
            ...
            console.log('[NewTicketTemplateQuiz] Selected answer:', {
            ...
            console.error('[NewTicketTemplateQuiz] Error handling answer:', error);
    ...
    async endGame(failed = false) {
        console.log('[NewTicketTemplateQuiz] Ending game...');
        ...
        try {
            ...
            console.log('[NewTicketTemplateQuiz] Quiz results:', {
        ...
        if (reviewList) {
            ...
            console.error('[NewTicketTemplateQuiz] Error ending game:', error);
    ...
    // Restart the quiz
    async restartQuiz() {
        console.log('[NewTicketTemplateQuiz] Restarting quiz...');
    ...
    showError(message) {
        console.error('[NewTicketTemplateQuiz] Error:', message);
    ...
    getTimerStorageKey() {
        ...
        return `ticket-template_timer_${username}_q${questionIndex}`;
    }
    ...
    saveTimerState(timeRemaining) {
        try {
            ...
            console.warn('[NewTicketTemplateQuiz] Failed to save timer state:', error);
        }
    }
    ...
    restoreTimerState() {
        try {
            ...
            if (timerData.questionIndex !== currentQuestionIndex) {
                console.log('[NewTicketTemplateQuiz] Timer data is for different question, ignoring');
            ...
            if (Date.now() - timerData.timestamp > maxAge) {
                console.log('[NewTicketTemplateQuiz] Timer data is too old, ignoring');
                localStorage.removeItem(storageKey);
                return null;
            }
            ...
            console.log(`[NewTicketTemplateQuiz] Restored timer state: ${timerData.timeRemaining}s remaining`);
            return timerData.timeRemaining;
            
        } catch (error) {
            console.warn('[NewTicketTemplateQuiz] Failed to restore timer state:', error);
            return null;
        }
    }
    ...
    clearCurrentTimerState() {
        try {
            const storageKey = this.getTimerStorageKey();
            localStorage.removeItem(storageKey);
            console.log('[NewTicketTemplateQuiz] Cleared current timer state');
        } catch (error) {
            console.warn('[NewTicketTemplateQuiz] Failed to clear timer state:', error);
        }
    }
    ...
    clearAllTimerStates() {
        try {
            const username = localStorage.getItem('username') || 'anonymous';
            const prefix = `ticket-template_timer_${username}_`;
            ...
            console.log(`[NewTicketTemplateQuiz] Cleared ${keysToRemove.length} timer states`);
        } catch (error) {
            console.warn('[NewTicketTemplateQuiz] Failed to clear timer states:', error);
        }
    }
    ...
    saveCurrentTimerState() {
        ...
        try {
            ...
            console.error('[NewTicketTemplateQuiz] Error saving current timer state:', error);
        }
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('[NewTicketTemplateQuiz] DOM loaded, initializing quiz...');
    window.newTicketTemplateQuiz = new NewTicketTemplateQuiz();
}); 