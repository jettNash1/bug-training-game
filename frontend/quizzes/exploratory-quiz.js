...
export class ExploratoryQuiz extends BaseQuiz {
    constructor() {
        console.log('[ExploratoryQuiz] Initializing...');
        ...
        const config = {
            ...
            performanceThresholds: [
                ...
                { threshold: 80, message: '👏 Great job! You\'ve shown strong exploratory testing instincts!' },
    ...
    async startGameWhenReady() {
        console.log('[ExploratoryQuiz] Waiting for timer settings to be loaded...');
        ...
        const checkTimerSettings = () => {
            attempts++;
            console.log(`[ExploratoryQuiz] Check attempt ${attempts}, timePerQuestion: ${this.timePerQuestion}`);
            ...
            if (this.timePerQuestion !== undefined && this.timePerQuestion !== null) {
                console.log(`[ExploratoryQuiz] Timer settings loaded: ${this.timePerQuestion}s per question`);
            ...
            if (attempts >= maxAttempts) {
                console.warn('[ExploratoryQuiz] Timer settings not loaded after maximum attempts, using BaseQuiz value');
    ...
    async startGame() {
        ...
        console.log('[ExploratoryQuiz] Starting game...');
        ...
        try {
            ...
            console.log(`[ExploratoryQuiz] Progress loaded: ${hasProgress}`);
            ...
            if (!hasProgress) {
                ...
                console.log('[ExploratoryQuiz] No previous progress, starting fresh');
            } else {
                ...
                if (!this.player.questionHistory || !Array.isArray(this.player.questionHistory)) {
                    console.log('[ExploratoryQuiz] Invalid question history in loaded progress, resetting');
                    this.player.questionHistory = [];
                }
                
                // CRITICAL: Ensure currentScenario is set correctly based on question history
                this.player.currentScenario = this.player.questionHistory.length;
                console.log('[ExploratoryQuiz] Set currentScenario to match question history:', this.player.currentScenario);
            }
            ...
            console.error('[ExploratoryQuiz] Error starting game:', error);
            this.isLoading = false;
    ...
    initializeTimer() {
        console.log(`[ExploratoryQuiz] Initializing timer for question ${this.player.questionHistory.length}`);
        ...
        if (!timerContainer || !timerDisplay) {
            console.error('[ExploratoryQuiz] Timer elements not found');
            return;
        }
        
        // Check if timer is disabled (0 seconds) or timer functionality is disabled
        if (this.timerDisabled || this.timePerQuestion === 0) {
            console.log(`[ExploratoryQuiz] Timer is disabled, hiding timer display`);
        ...
        if (restoredTime !== null) {
            ...
            console.log(`[ExploratoryQuiz] Restored timer with ${timeLeft} seconds remaining`);
        } else {
            timeLeft = this.timePerQuestion;
            console.log(`[ExploratoryQuiz] Starting new timer with ${timeLeft} seconds`);
        }
    ...
    handleTimedOut() {
        console.log('[ExploratoryQuiz] Question timed out');
    ...
    displayScenario() {
        ...
        console.log(`[ExploratoryQuiz] Displaying scenario #${this.player.currentScenario + 1}:`, {
        ...
        if (this.player.questionHistory.length > 0) {
            ...
            this.saveProgress('in-progress').catch(err => {
                console.warn('[ExploratoryQuiz] Save after display failed:', err);
            });
        }
    }
    ...
    async handleAnswer(timedOut = false) {
        ...
        if (submitButton) {
            ...
            console.log('[ExploratoryQuiz] Selected answer:', {
            ...
            console.error('[ExploratoryQuiz] Error handling answer:', error);
    ...
    async endGame(failed = false) {
        console.log('[ExploratoryQuiz] Ending game...');
        ...
        try {
            ...
            console.log('[ExploratoryQuiz] Quiz results:', {
        ...
        if (reviewList) {
            ...
            if (recommendations) {
                ...
                if (scorePercentage >= 90) {
                    recommendationsHTML = `
                        <p>🌟 Outstanding! You have demonstrated excellent knowledge of exploratory testing principles!</p>
                        <p>You show strong understanding of:</p>
                        <ul>
                            <li>Test planning and structured exploration</li>
                            <li>Time management and session-based testing</li>
                            <li>Documentation and note-taking practices</li>
                            <li>Bug reporting and defect analysis</li>
                            <li>Advanced testing techniques and strategies</li>
                        </ul>
                    `;
                    ...
                    recommendationsHTML = `
                        <p>📚 Focus on these fundamental areas for improvement:</p>
                        <ul>
                            <li><strong>Basic Concepts:</strong><ul>
                    `;
                    // Find areas where the user made mistakes, prioritizing basic concepts
                    const incorrectQuestions = this.player.questionHistory.filter(q => !q.isCorrect);
                    const basicQuestions = incorrectQuestions.filter(q => q.scenario.level === 'Basic');
                    const otherQuestions = incorrectQuestions.filter(q => q.scenario.level !== 'Basic');
                    ...
                    if (otherQuestions.length > 0) {
                        recommendationsHTML += `<li><strong>Additional Areas:</strong><ul>`;
            ...
            console.error('[ExploratoryQuiz] Error ending game:', error);
    ...
    // Restart the quiz
    async restartQuiz() {
        console.log('[ExploratoryQuiz] Restarting quiz...');
    ...
    showError(message) {
        console.error('[ExploratoryQuiz] Error:', message);
    ...
    getTimerStorageKey() {
        ...
        return `exploratory_timer_${username}_q${questionIndex}`;
    }
    ...
    saveTimerState(timeRemaining) {
        try {
            ...
            console.warn('[ExploratoryQuiz] Failed to save timer state:', error);
        }
    }
    ...
    restoreTimerState() {
        try {
            ...
            if (timerData.questionIndex !== currentQuestionIndex) {
                console.log('[ExploratoryQuiz] Timer data is for different question, ignoring');
            ...
            if (Date.now() - timerData.timestamp > maxAge) {
                console.log('[ExploratoryQuiz] Timer data is too old, ignoring');
                localStorage.removeItem(storageKey);
                return null;
            }
            ...
            console.log(`[ExploratoryQuiz] Restored timer state: ${timerData.timeRemaining}s remaining`);
            return timerData.timeRemaining;
            
        } catch (error) {
            console.warn('[ExploratoryQuiz] Failed to restore timer state:', error);
            return null;
        }
    }
    ...
    clearCurrentTimerState() {
        try {
            const storageKey = this.getTimerStorageKey();
            localStorage.removeItem(storageKey);
            console.log('[ExploratoryQuiz] Cleared current timer state');
        } catch (error) {
            console.warn('[ExploratoryQuiz] Failed to clear timer state:', error);
        }
    }
    ...
    clearAllTimerStates() {
        try {
            const username = localStorage.getItem('username') || 'anonymous';
            const prefix = `exploratory_timer_${username}_`;
            ...
            console.log(`[ExploratoryQuiz] Cleared ${keysToRemove.length} timer states`);
        } catch (error) {
            console.warn('[ExploratoryQuiz] Failed to clear timer states:', error);
        }
    }
    ...
    saveCurrentTimerState() {
        ...
        try {
            ...
            console.error('[ExploratoryQuiz] Error saving current timer state:', error);
        }
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('[ExploratoryQuiz] DOM loaded, initializing quiz...');
    window.exploratoryQuiz = new ExploratoryQuiz();
}); 