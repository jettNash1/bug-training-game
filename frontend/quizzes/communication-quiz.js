...
export class CommunicationQuiz extends BaseQuiz {
    constructor() {
        console.log('[CommunicationQuiz] Initializing...');
        ...
        const config = {
            ...
            performanceThresholds: [
                ...
                { threshold: 80, message: '👏 Great job! You\'ve shown strong communication instincts!' },
    ...
    async startGameWhenReady() {
        console.log('[CommunicationQuiz] Waiting for timer settings to be loaded...');
        ...
        while (attempts < maxAttempts) {
            if (this.timePerQuestion !== undefined && this.timePerQuestion !== null) {
                console.log('[CommunicationQuiz] Timer settings ready, starting game...');
                break;
            }
        ...
        if (attempts >= maxAttempts) {
            console.warn('[CommunicationQuiz] Timeout waiting for timer settings, proceeding with defaults');
        }
    ...
    async startGame() {
        ...
        console.log('[CommunicationQuiz] Starting game...');
    ...
    initializeTimer() {
        console.log(`[CommunicationQuiz] Initializing timer for question ${this.player.questionHistory.length}`);
        ...
        if (!timerContainer || !timerDisplay) {
            console.error('[CommunicationQuiz] Timer elements not found');
            return;
        }
        
        // Check if timer is disabled (0 seconds) or timer functionality is disabled
        if (this.timerDisabled || this.timePerQuestion === 0) {
            console.log(`[CommunicationQuiz] Timer is disabled, hiding timer display`);
        ...
        if (restoredTime !== null) {
            ...
            console.log(`[CommunicationQuiz] Restored timer with ${timeLeft} seconds remaining`);
        } else {
            timeLeft = this.timePerQuestion;
            console.log(`[CommunicationQuiz] Starting new timer with ${timeLeft} seconds`);
        }
    ...
    handleTimedOut() {
        console.log('[CommunicationQuiz] Question timed out');
    ...
    displayScenario() {
        ...
        console.log(`[CommunicationQuiz] Displaying scenario #${this.player.currentScenario + 1}:`, {
    ...
    async handleAnswer(timedOut = false) {
        ...
        if (submitButton) {
            ...
            console.error('[CommunicationQuiz] Error handling answer:', error);
    ...
    async endGame(failed = false) {
        console.log('[CommunicationQuiz] Ending game...');
        ...
        try {
            ...
            console.log('[CommunicationQuiz] Quiz results:', {
        ...
        if (reviewList) {
            ...
            if (recommendations) {
                ...
                if (scorePercentage >= 90) {
                    recommendationsHTML = '<p>🌟 Outstanding! You have demonstrated excellent knowledge of communication principles!</p>';
                    ...
                    Object.entries(groupedByLevel).forEach(([level, questions]) => {
                        ...
                        questions.forEach(q => {
                            recommendationsHTML += `<li>${q.scenario.title}: ${q.scenario.description}<br>
                                <em>Recommendation: ${q.selectedAnswer.outcome}</em></li>`;
                        });
                        recommendationsHTML += '</ul></li>';
                    });
                    recommendationsHTML += '</ul>';
            ...
            console.error('[CommunicationQuiz] Error ending game:', error);
    ...
    // Restart the quiz
    async restartQuiz() {
        console.log('[CommunicationQuiz] Restarting quiz...');
    ...
    showError(message) {
        console.error('[CommunicationQuiz] Error:', message);
    ...
    getTimerStorageKey() {
        ...
        return `communication_timer_${username}_q${this.player.questionHistory.length}`;
    }
    ...
    saveTimerState(timeRemaining) {
        try {
            ...
            // console.log(`[CommunicationQuiz] Timer state saved: ${timeRemaining}s remaining for question ${this.player.questionHistory.length}`);
        } catch (error) {
            console.error('[CommunicationQuiz] Error saving timer state:', error);
        }
    }
    ...
    restoreTimerState() {
        try {
            ...
            if (!timerDataStr) {
                console.log('[CommunicationQuiz] No timer state found to restore');
                return null;
            }
            ...
            if (!timerData || 
                ...
                console.warn('[CommunicationQuiz] Invalid timer state data, removing');
            ...
            if (timerData.questionIndex !== this.player.questionHistory.length) {
                console.log(`[CommunicationQuiz] Timer state is for question ${timerData.questionIndex}, but current is ${this.player.questionHistory.length}. Clearing old state.`);
                localStorage.removeItem(storageKey);
                return null;
            }
            ...
            if (timeDiff > 10 * 60 * 1000) { // 10 minutes
                console.log('[CommunicationQuiz] Timer state is too old, removing');
                localStorage.removeItem(storageKey);
                return null;
            }
            ...
            if (timerData.timeRemaining <= 0 || timerData.timeRemaining > this.timePerQuestion) {
                console.log('[CommunicationQuiz] Invalid time remaining in stored state, removing');
                localStorage.removeItem(storageKey);
                return null;
            }
            
            console.log(`[CommunicationQuiz] Restored timer state: ${timerData.timeRemaining}s remaining for question ${timerData.questionIndex}`);
            return timerData.timeRemaining;
        } catch (error) {
            console.error('[CommunicationQuiz] Error restoring timer state:', error);
            return null;
        }
    }
    ...
    clearCurrentTimerState() {
        try {
            const storageKey = this.getTimerStorageKey();
            localStorage.removeItem(storageKey);
            console.log(`[CommunicationQuiz] Cleared timer state for question ${this.player.questionHistory.length}`);
        } catch (error) {
            console.error('[CommunicationQuiz] Error clearing timer state:', error);
        }
    }
    ...
    clearAllTimerStates() {
        try {
            const username = localStorage.getItem('username') || 'anonymous';
            const prefix = `communication_timer_${username}_`;
            ...
            console.log(`[CommunicationQuiz] Cleared ${keysToRemove.length} timer states`);
        } catch (error) {
            console.error('[CommunicationQuiz] Error clearing all timer states:', error);
        }
    }
    ...
    saveCurrentTimerState() {
        ...
        try {
            ...
            console.error('[CommunicationQuiz] Error saving current timer state:', error);
        }
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.communicationQuiz = new CommunicationQuiz();
}); 